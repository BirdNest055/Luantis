...
local function test_mapgen_edges(cb)
	...
	local function emerge_block(blockpos, action, blocks_left, finished)
		-- EMERGE_CANCELLED can mean either:
		-- 1. The block wasn't emerged (cancelled before completion)
		-- 2. The block was already being emerged (so it IS emerged now)
		-- We can't reliabily distinguish these cases in the callback,
		-- so we check if the block actually exists before counting it
		if action == core.EMERGE_CANCELLED then
			-- Check if block exists (already emerged)
			local node = core.get_node_or_nil(blockpos)
