...
- **`clawtest-v9.57-fix-todo-items-b22-27` branch:** Current — 170+ improvements across batches 22-33: crash prevention, memory safety/RAII, protocol robustness, Lua API safety, mapgen correctness, I/O hardening, string safety, container bounds, integer overflow, rendering safety, network hardening. Build tested: both client AND server compile with zero errors.
...
| v9.57 | `clawtest-v9.57-fix-todo-items-b22-27` | 290+ improvements across batches 22-39 — crash prevention/null deref hardening, memory safety/RAII, protocol robustness/race conditions, Lua API safety/input validation, mapgen correctness/world integrity, I/O hardening/rendering safety, string safety/format validation, container bounds/iterator safety, integer overflow/arithmetic safety, file I/O error handling, client rendering/UI safety, protocol/network hardening, thread safety/concurrency, resource cleanup/destructor safety, configuration/settings validation, map/world data integrity, script API hardening, client input/event safety; client+server build testing (both compile with zero errors) |
