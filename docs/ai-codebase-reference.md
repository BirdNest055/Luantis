...
| v9.57 | `clawtest-v9.57-fix-todo-items-b22-27` | 170+ improvements across batches 22-33 — crash prevention/null deref hardening, memory safety/RAII, protocol robustness/race conditions, Lua API safety/input validation, mapgen correctness/world integrity, I/O hardening/rendering safety, string safety/format validation, container bounds/iterator safety, integer overflow/arithmetic safety, file I/O error handling, client rendering/UI safety, protocol/network hardening; client+server build testing (both compile with zero errors) |
