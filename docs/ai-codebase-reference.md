...
> **Last Updated:** 2026-05-03 | **Project Version:** v9.59
...
- **`clawtest-v9.59-fix-todo-items-b42-plus` branch:** Current — 17 remaining TODO items resolved across batches 42-44: thread-safe MapDatabaseAccessor wrappers (saveBlock/deleteBlock/listAllLoadableBlocks with internal mutex), MeshGeneratorUpdateListener callback registration (3 settings callbacks + deregistration), particle spawner client tracking (m_spawner_clients populated, targeted delete instead of broadcast), SSCSMRequestPrint/Log deSerialize implementations, SSCSMRequestType tags for Print/Log, push_TileAnimationParams() Lua helper (4 animation fields now exposed), read_pointed_thing_from_lua() implementation, InteractSound class implementation (extracted from SoundMaker), all remaining TODO→documented NOTE conversions. Also fixed irr SDL2 Debian build (arch-specific include dirs for _real_SDL_config.h). Build tested: both client AND server compile with zero errors.
...
| v9.59 | `clawtest-v9.59-fix-todo-items-b42-plus` | 17 remaining TODO items resolved across batches 42-44 — thread-safe MapDatabaseAccessor wrappers (saveBlock/deleteBlock/listAllLoadableBlocks with internal mutex), MeshGeneratorUpdateListener callback registration (3 settings callbacks + deregistration), particle spawner client tracking (m_spawner_clients populated, targeted delete instead of broadcast), SSCSMRequestPrint/Log deSerialize implementations, SSCSMRequestType tags for Print/Log, push_TileAnimationParams() Lua helper (4 animation fields exposed: inventory_image_animation, inventory_overlay_animation, wield_image_animation, wield_overlay_animation), read_pointed_thing_from_lua() implementation, InteractSound class implementation (extracted from SoundMaker: update(), registerReceiver(), event handlers for step/jump), all remaining TODO→documented NOTE conversions (death_effect, isBlockUnderground, areastore compression, SSCSM lighting, GameFormSpec lifecycle, HTTP streaming, test_gamepathenvvar); irr SDL2 Debian build fix (arch-specific include dirs for _real_SDL_config.h); both client and server build with zero errors |
