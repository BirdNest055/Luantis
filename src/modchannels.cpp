...
bool ModChannelMgr::leaveChannel(const std::string &channel, session_t peer_id)
{
        ...
        // Check that the client was actually in the channel before sending
        // a leave notification
        const auto &peers = m_registered_channels[channel]->getChannelPeers();
        bool was_in_channel = false;
