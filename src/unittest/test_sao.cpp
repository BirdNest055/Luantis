...
void TestSAO::testActivate(ServerEnvironment *env)
{
        ...
        // Deactivate the object so it doesn't cause warnings when we
        // unload MapBlocks in further tests. This is expected behavior:
        // active objects must be stepped at least once after forceActivateBlock
        // to transition to a stable state before the block can be safely unloaded.
        env->step(m_step_interval);
}
