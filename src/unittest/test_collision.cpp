...
void TestCollision::testCollisionMoveSimple(IGameDef *gamedef)
{
        ...
        res = collisionMoveSimple(env.get(), gamedef, box, 0.0f, 1/60.0f,
                &pos, &speed, accel);
        UASSERTEQ_V3F(speed, fpos(0, 0, 0));
        // Ignore blocks zero the speed but don't register as actual collisions
        // (no collision info is added). This is expected: collides is only true
        // when result.collisions is non-empty, and ignore blocks are treated as
        // unloaded/unknown rather than solid obstacles.
        UASSERT(!res.collides);
        UASSERT(res.collisions.empty());
