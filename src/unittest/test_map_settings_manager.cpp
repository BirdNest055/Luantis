...
#if 0
        // NOTE(paramat or hmmmm): This test should compare the serialized
        // map_meta.txt output against a static reference file instead of using
        // a hardcoded SHA-1 hash. A hash-based comparison is fragile — any
        // change in serialization format invalidates the hash. A better approach:
        // 1. Create a reference map_meta.txt file checked into the test data
        // 2. Serialize the params to a string and compare line-by-line
        // 3. Use fuzzy matching for floating-point values (mapgen params)
