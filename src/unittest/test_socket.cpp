...
void TestSocket::testIPv4Socket()
{
        ...
        //NOTE: This fails on some systems (e.g. CI containers without loopback,
        // firewalled environments, or Windows with strict firewall rules) because
        // UDP loopback is not guaranteed. The 50ms sleep may also be insufficient
        // on slow or heavily loaded systems. A more robust approach would use a
        // retry loop with timeout instead of a single Receive pass.
...
void TestSocket::testIPv6Socket()
{
...

        {
                ...
                //NOTE: This fails on some systems (e.g. CI containers without IPv6
                // loopback, firewalled environments) because IPv6 UDP loopback is
                // even less reliably available than IPv4. The 50ms sleep may also be
                // insufficient on slow or heavily loaded systems.
                UASSERT(strncmp(sendbuffer, rcvbuffer, sizeof(sendbuffer)) == 0);
