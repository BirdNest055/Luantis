...
void TestMapDatabase::testRemove()
{
        ...
        // failed remove
        // NOTE: deleteBlock() return value for non-existent keys is inconsistent
        // across database backends (SQLite3 returns true, Dummy returns false).
        // This cannot be tested uniformly until the contract is clarified:
        // should deleteBlock() return false when the key doesn't exist, or
        // should it always return true (idempotent delete)? Once the spec is
        // agreed upon, all backends must be updated to match, and this test
        // can be enabled.
