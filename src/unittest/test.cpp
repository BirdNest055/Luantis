...
struct TestMapBlock: public TestBase
{
        ...
        void Run()
        {
                ...
                /*
                        NOTE: getSizeNodes() was removed because block size is
                        not variable (fixed at MAP_BLOCKSIZE). If variable block
                        sizes are ever implemented, this test should be
                        re-enabled with the appropriate assertion.
                */
                ...
                */
                ...
                // NOTE: getNodeTile() was replaced by the tile-def system.
                // The old API used a single content-ID lookup; the new system
                // resolves tiles through NodeDefManager::getNodeTile() with
                // per-face tile definitions. These assertions need to be
                // rewritten to use the new tile lookup API.
...
struct TestMapSector: public TestBase
{
        ...
        void Run()
        {
                ...
                // NOTE: Should test that createBlankBlock() throws
                // AlreadyExistsException when called twice with the same Y.
                // Currently disabled because createBlankBlock may silently
                // return nullptr instead of throwing. To enable:
                // 1. Verify that ServerMapSector::createBlankBlock() can throw
                // 2. Write: UASSERT_THROWS(sector.createBlankBlock(-2), ...)
                // 3. If it doesn't throw, the API should be updated to do so.
