...
void compressZstd(const u8 *data, size_t data_size, std::ostream &os, int level)
{
        ...
        while (input.pos < input.size) {
                ...
                if (output.pos) {
                        ...
                        if (os.fail()) {
                                throw SerializationError("compressZstd: write failed");
                        }
                        output.pos = 0;
                }
        }
...
void decompressZstd(std::istream &is, std::ostream &os)
{
        ...
        const size_t bufsize = 16384;
        const size_t max_output_size = 100 * 1024 * 1024; // 100 MB limit to prevent decompression bombs
        char output_buffer[bufsize];
        char input_buffer[bufsize];
        ...
        do
        {
                ...
                if (output.pos) {
                        ...
                        if (total_output > max_output_size) {
                                throw SerializationError("decompressZstd: output exceeds 100 MB limit (possible decompression bomb)");
                        }
                        os.write(output_buffer, output.pos);
                        output.pos = 0;
                }
