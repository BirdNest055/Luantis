...
int MetaDataRef::l_get_float(lua_State *L)
{
        ...
        auto parsed = my_string_to_double(str);
        f64 number = parsed.value_or(0.0);
        if (!parsed.has_value() && !str.empty()) {
                warningstream << "MetaDataRef::get_float(): key \"" << name
                        << "\" has non-numeric value \"" << str
