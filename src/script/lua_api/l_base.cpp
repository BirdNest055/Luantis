...
{
        // Check whether a function with the same name already exists in the table.
        // If it does, log a warning since a duplicate registration silently replaces
        // the previous function, which can cause hard-to-debug issues.
        lua_getfield(L, top, name);
        if (!lua_isnil(L, -1)) {
                warningstream << "registerFunction(): function '" << name
