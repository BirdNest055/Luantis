...
// get_connection_stats() — v9.38: connection statistics for server info overlay
int ModApiClient::l_get_connection_stats(lua_State *L)
{
        Client *client = getClient(L);
        lua_newtable(L);
        // RTT in milliseconds
        float rtt = client->getRTT();
        lua_pushnumber(L, rtt * 1000.0f);
        ...
        lua_setfield(L, -2, "address");
        // Server port
        Address addr = client->getServerAddress();
        lua_pushinteger(L, addr.getPort());
        lua_setfield(L, -2, "port");
        return 1;
}
