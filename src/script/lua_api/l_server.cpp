...
int ModApiServer::l_request_shutdown(lua_State *L)
{
        ...
        const char *msg = lua_isstring(L, 1) ? lua_tolstring(L, 1, NULL) : NULL; // Batch 38: type check before lua_tolstring
        bool reconnect = readParam<bool>(L, 2);
        float seconds_before_shutdown = lua_isnumber(L, 3) ? lua_tonumber(L, 3) : 0.0f; // Batch 38: type check before lua_tonumber
