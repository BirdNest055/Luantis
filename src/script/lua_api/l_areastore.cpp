...
int LuaAreaStore::l_to_file(lua_State *L)
{
        ...
        // Batch 31: Validate filename for path traversal (defense in depth)
        std::string fn(filename);
        if (fn.find("..") != std::string::npos) {
                warningstream << "l_to_file: path traversal attempt in filename: "
                        << fn << std::endl;
                lua_pushboolean(L, false);
                return 1;
        }
...
int LuaAreaStore::l_from_file(lua_State *L)
{
        ...
        std::string fn(filename);
        if (fn.find("..") != std::string::npos) {
                warningstream << "l_from_file: path traversal attempt in filename: "
                        << fn << std::endl;
                lua_pushboolean(L, false);
                return 1;
        }
