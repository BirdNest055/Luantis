...
int LuaVoxelManip::l_set_data(lua_State *L)
{
        ...
        for (u32 i = 0; i != volume; i++) {
                lua_rawgeti(L, 2, i + 1);
                content_t c = lua_isnumber(L, -1) ? lua_tointeger(L, -1) : CONTENT_IGNORE; // Batch 38: type check
...
int LuaVoxelManip::l_set_light_data(lua_State *L)
{
        ...
        for (u32 i = 0; i != volume; i++) {
                lua_rawgeti(L, 2, i + 1);
                u8 light = lua_isnumber(L, -1) ? lua_tointeger(L, -1) : 0; // Batch 38: type check
...
int LuaVoxelManip::l_set_param2_data(lua_State *L)
{
        ...
        for (u32 i = 0; i != volume; i++) {
                lua_rawgeti(L, 2, i + 1);
                u8 param2 = lua_isnumber(L, -1) ? lua_tointeger(L, -1) : 0; // Batch 38: type check
