...
/******************************************************************************/
/* v9.29: Keypair authentication API for mainmenu Lua */
int ModApiMainMenu::l_keypair_is_enabled(lua_State *L)
{
        lua_pushboolean(L, g_settings->getBool("keypair_auth"));
        return 1;
}
...
int ModApiMainMenu::l_keypair_get_server_user(lua_State *L)
{
        std::string server_addr = luaL_checkstring(L, 1);
        if (!g_settings->getBool("keypair_auth")) {
                lua_pushstring(L, "");
                return 1;
        }
        KeypairManager km(porting::path_user);
        lua_pushstring(L, km.getServerUser(server_addr).c_str());
        return 1;
}

int ModApiMainMenu::l_keypair_has_server_user(lua_State *L)
{
        std::string server_addr = luaL_checkstring(L, 1);
        if (!g_settings->getBool("keypair_auth")) {
                lua_pushboolean(L, false);
        ...
        KeypairManager km(porting::path_user);
        lua_pushboolean(L, km.hasServerUser(server_addr));
        return 1;
}
...
int ModApiMainMenu::l_keypair_get_server_list(lua_State *L)
{
        if (!g_settings->getBool("keypair_auth")) {
                lua_newtable(L);
                return 1;
        ...
        KeypairManager km(porting::path_user);
        auto server_list = km.getServerUserList();
...
int ModApiMainMenu::l_keypair_forget_server(lua_State *L)
{
        std::string server_addr = luaL_checkstring(L, 1);
        if (!g_settings->getBool("keypair_auth")) {
                lua_pushboolean(L, false);
        ...
        KeypairManager km(porting::path_user);
        lua_pushboolean(L, km.forgetServerUser(server_addr));
        return 1;
}
...
void ModApiMainMenu::Initialize(lua_State *L, int top)
{
        ...
        API_FCT(keypair_is_enabled);
        API_FCT(keypair_has_keypair);
        API_FCT(keypair_get_server_user);
        API_FCT(keypair_has_server_user);
        API_FCT(keypair_get_public_key_base64);
        API_FCT(keypair_regenerate);
