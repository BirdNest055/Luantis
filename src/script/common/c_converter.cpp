...
video::SColor read_ARGB8(lua_State *L, int index)
{
        ...
        // Validate that the table looks like a color table before reading.
        // This mirrors the checks in is_color_table() for consistency.
        CHECK_TYPE(index, "ARGB color", LUA_TTABLE);
        lua_getfield(L, index, "r");
        if (!lua_isnumber(L, -1))
                throw LuaError("ARGB color: field 'r' must be a number");
        lua_getfield(L, index, "g");
        if (!lua_isnumber(L, -1))
                throw LuaError("ARGB color: field 'g' must be a number");
        lua_getfield(L, index, "b");
        if (!lua_isnumber(L, -1))
                throw LuaError("ARGB color: field 'b' must be a number");
        lua_pop(L, 3); // pop b, g, r — we'll read them again below
