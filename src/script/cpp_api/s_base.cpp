...
Server* ScriptApiBase::getServer()
{
        ...
        // Thread-safety assertion: at minimum, when called from the server
        // scripting type, verify it's on the server thread.
        if (getType() == ScriptingType::Server) {
                // This should only be called from the server thread when in
                // server scripting mode. In the async environment this check
