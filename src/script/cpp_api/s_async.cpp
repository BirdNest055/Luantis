...
bool AsyncEngine::prepareEnvironment(lua_State* L, int top)
{
        ...
        try {
                ...
                // NOTE: There is no general way to report fatal errors from an async
                // environment back to its owner (e.g. GUIEngine, or the server thread).
                // Currently we only handle the server case via server->setAsyncFatalError().
                //
                // Root cause: AsyncScriptEnvironment is used by multiple owners (server-side
                // async envs and GUIEngine-side async envs), but there is no common error-
                // reporting interface. Each owner would need its own error callback.
                //
                // Proposed solution: Add an error callback (e.g. std::function<void(const
                // std::string&)> or a virtual interface) to AsyncScriptEnvironment's
                // constructor. Each owner (Server, GUIEngine) would provide its own
                // implementation. This would replace the current ad-hoc server check and
                // provide a uniform error path for all async environments.
                return false;
        }
