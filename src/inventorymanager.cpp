...
void IMoveAction::onTake(const ItemStack &src_item, ServerActiveObject *player) const
{
        ...
        if (from_inv.type == InventoryLocation::DETACHED)
                ...
                FATAL_ERROR("Unknown inventory location type");
}
...
void IMoveAction::apply(InventoryManager *mgr, ServerActiveObject *player, IGameDef *gamedef)
{
        ...
        if (!list_from) {
                warningstream << "IMoveAction::apply(): FAIL: source inventory or list not found: "
        ...
        if (!list_to) {
                warningstream << "IMoveAction::apply(): FAIL: destination inventory or list not found: "
                                << "from_inv=\"" << from_inv.dump() << "\""
                                << ", from_list=\"" << from_list << "\""
                                << ", to_inv=\"" << to_inv.dump() << "\""
                                << ", to_list=\"" << to_list << "\""
        ...
        if (from_i < 0 || list_from->getSize() <= (u32) from_i) {
                warningstream << "IMoveAction::apply(): FAIL: source index out of bounds: "
        ...
        if (to_i < 0 || list_to->getSize() <= (u32) to_i) {
                warningstream << "IMoveAction::apply(): FAIL: destination index out of bounds: "
...
void IDropAction::apply(InventoryManager *mgr, ServerActiveObject *player, IGameDef *gamedef)
{
        ...
        if (!inv_from) {
                warningstream<<"IDropAction::apply(): FAIL: source inventory not found: "
        ...
        */
        if (!list_from) {
                warningstream<<"IDropAction::apply(): FAIL: source list not found: "
                                <<"from_inv=\""<<from_inv.dump()<<"\""<<std::endl;
                return;
        }
        if (from_i < 0 || list_from->getSize() <= (u32) from_i) {
        ...
        ItemStack src_item = list_from->getItem(from_i);
        if (src_item.empty()) {
                warningstream<<"IDropAction::apply(): FAIL: source item not found: "
                                <<"from_inv=\""<<from_inv.dump()<<"\""
                                <<", from_list=\""<<from_list<<"\""
        ...
        ServerActiveObject *player, IGameDef *gamedef)
{
        ...
        if (!inv_craft) {
                warningstream << "ICraftAction::apply(): FAIL: inventory not found: "
        ...
        */
        if (!list_craft) {
                warningstream << "ICraftAction::apply(): FAIL: craft list not found: "
                                << "craft_inv=\"" << craft_inv.dump() << "\"" << std::endl;
                return;
        }
        if (!list_craftresult) {
                warningstream << "ICraftAction::apply(): FAIL: craftresult list not found: "
                                << "craft_inv=\"" << craft_inv.dump() << "\"" << std::endl;
                return;
        }
        if (list_craftresult->getSize() < 1) {
                warningstream << "ICraftAction::apply(): FAIL: craftresult list too short: "
                                << "craft_inv=\"" << craft_inv.dump() << "\"" << std::endl;
                return;
        }
