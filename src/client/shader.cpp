...
public:
        ...
        void onGenerate(const std::string &name, ShaderConstants &constants) override
        {
                ...
                if (g_settings->getBool("enable_dynamic_shadows")) {
                        ...
                        // Node specular effect - enabled per setting
                        // NOTE #30: Node specular effect is currently applied globally via a
                        // shader constant (ENABLE_NODE_SPECULAR). It should be per-node so
                        // that only nodes with specular materials incur the lighting cost.
                        // See issue #15898.
                        // Root cause: The shader has no per-node material metadata. All nodes
                        // in a draw call share the same shader constants. Enabling specular
                        // globally means the fragment shader computes specular for all nodes
                        // even when most don't need it.
                        // Proposed fix: (1) Encode a specular flag in the vertex color alpha
                        // channel or in a custom vertex attribute. (2) In the fragment shader,
                        // check this flag before computing specular. (3) The mesh generator
                        // would set the flag based on the node's material definition. This
                        // requires changes to mapblock_mesh.cpp and the shader source.
