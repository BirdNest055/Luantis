...
{
        if (texture_a >= m_definitions.size() || texture_b >= m_definitions.size() ||
                        !m_definitions[texture_a].valid || !m_definitions[texture_b].valid) {
                errorstream << "TextureBuffer::swapTextures(): invalid texture indices "
                        << (int)texture_a << ", " << (int)texture_b << std::endl;
                return;
        }
