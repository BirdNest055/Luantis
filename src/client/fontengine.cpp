...
void FontEngine::fontSettingChanged(const std::string &name, void *userdata)
{
        ...
        ((FontEngine *)userdata)->m_needs_reload.store(true, std::memory_order_relaxed);
}
...
{
        // Batch 34: Atomic load — m_needs_reload can be set from any thread
        if (!m_needs_reload.load(std::memory_order_relaxed))
                return;

        m_needs_reload.store(false, std::memory_order_relaxed);
        readSettings();
}
