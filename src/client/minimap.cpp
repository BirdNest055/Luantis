...
video::ITexture *Minimap::getMinimapTexture()
{
        ...
        case MINIMAP_TYPE_RADAR:
                ...
                break;
        case MINIMAP_TYPE_TEXTURE:
                // NOTE: GPU→CPU→GPU roundtrip — texture data is read back from the GPU via
                // createImageFromData, then copied onto map_image, which is uploaded again.
                // Root cause: Irrlicht does not expose blit-from-texture-to-texture or
                // render-to-texture for this use case. The minimap pipeline requires
                // compositing the texture onto map_image at an offset, which is done on the
                // CPU side via IImage::copyTo.
                // Proposed solution: Use an RTT (render-to-target) approach — render a
                // fullscreen quad with the texture sampled at the correct offset directly
                // onto the minimap render target, eliminating the readback entirely.
                // This requires refactoring the minimap rendering pipeline to work in GPU
                // space rather than CPU space for the TEXTURE mode case.
