...
void Client::deletingPeer(con::IPeer *peer, bool timeout)
{
        ...
        // Reset security info on disconnect — stale data should not persist
        m_security_info = ConnectionSecurityInfo();
        g_settings->set("security_info_state", "Not Connected");
        g_settings->set("security_info_encryption", "N/A");
        g_settings->set("security_info_key_exchange", "N/A");
        g_settings->set("security_info_authentication", "N/A");
        g_settings->set("security_info_cipher_suite", "N/A");
        g_settings->set("security_info_cert_status", "N/A");
        g_settings->set("security_info_forward_secrecy", "N/A");
        g_settings->set("security_info_replay_protection", "N/A");
        g_settings->set("security_info_protocol_version", "N/A");
        g_settings->set("security_info_server_address", "N/A");
        g_settings->set("security_info_server_port", "N/A");
        // v8: Reset new Security Info Tab fields
        g_settings->set("security_info_session_id", "N/A");
...
void Client::deleteAuthData()
{
        ...
        switch (m_chosen_auth_mech) {
                case AUTH_MECHANISM_FIRST_SRP:
                        // v9 fix: FIRST_SRP now also creates an SRPUser for encryption,
                        // so we must delete it the same way as SRP/LEGACY_PASSWORD.
                        if (m_auth_data) {
...
void Client::startAuth(AuthMechanism chosen_auth_mechanism)
{
        ...
        switch (chosen_auth_mechanism) {
                ...
                case AUTH_MECHANISM_FIRST_SRP: {
                        ...
                        // the verifier directly, which meant m_auth_data stayed NULL
                        // and handleCommand_AuthAccept() could never extract a session
                        // key — resulting in "SRPUser is NULL" and an INSECURE
                        // connection for every new-user registration.
                        //
                        // The fix: We still send the FIRST_SRP verifier packet for
                        // account creation, but then we ALSO send SRP_BYTES_A so the
                        // server can perform a full SRP key exchange after storing the
                        // verifier. The server now handles this by performing the SRP
                        // exchange after creating the account, instead of calling
                        // acceptAuth() immediately. Both sides end up with the same
                        // session key, enabling encryption.
                        std::string verifier;
                        std::string salt;
                        generate_srp_verifier_and_salt(playername, m_password,
                                &verifier, &salt);

                        // Send the verifier for account creation
                        NetworkPacket resp_pkt(TOSERVER_FIRST_SRP, 0);
                        resp_pkt << salt << verifier << (u8)((m_password.empty()) ? 1 : 0);
                        Send(&resp_pkt);

                        // Always initiate an SRP exchange so both client and server
                        // derive the same session key for encryption. Even for empty
                        // passwords, the SRP protocol works — the verifier is derived
                        // from the empty string, and both sides still get a shared key.
                        // The server will receive our SRP_BYTES_A, create an SRPVerifier
                        // from the just-stored credentials, and send back SrpBytesSandB.
                        {
                                ...
                                // v9.1 fix: Check that srp_user_new() succeeded.
                                // If memory allocation fails, m_auth_data will be NULL,
                                // which would cause a crash in srp_user_start_authentication()
                                // and later "SRPUser is NULL" in handleCommand_AuthAccept().
                                if (!m_auth_data) {
                                        errorstream << "srp_user_new() failed for FIRST_SRP: "
                                                << "memory allocation failure, connection will be insecure"
                                                << std::endl;
                                        break;
                                }
                                ...
                                // v9.1 fix: Handle srp_user_start_authentication() failure.
                                // Previously, failure was silently ignored, which meant the
                                // server never received SRP_BYTES_A, m_auth_data had no
                                // session key, and handleCommand_AuthAccept() would fail.
                                if (res != SRP_OK) {
                                        errorstream << "srp_user_start_authentication() failed for FIRST_SRP: "
                                                << "result=" << (int)res
                                                << ", connection will be insecure"
                                                << std::endl;
                                        srp_user_delete((struct SRPUser *) m_auth_data);
                                        m_auth_data = NULL;
                                        break;
                                }

                                if (bytes_A && len_A > 0) {
                                        NetworkPacket srp_pkt(TOSERVER_SRP_BYTES_A, 0);
                                        srp_pkt << std::string(bytes_A, len_A)
                                                << (u8)1 /* based_on = SRP */;
                                        Send(&srp_pkt);
                                } else {
                                        errorstream << "srp_user_start_authentication() returned no bytes_A for FIRST_SRP: "
                                                << "connection will be insecure"
                                                << std::endl;
                                        srp_user_delete((struct SRPUser *) m_auth_data);
                                        m_auth_data = NULL;
                ...
                case AUTH_MECHANISM_LEGACY_PASSWORD: {
                        ...
                        // v9.1 fix: Check that srp_user_new() succeeded before using it.
                        // If this returns NULL, proceeding would cause a crash in
                        // srp_user_start_authentication() and "SRPUser is NULL" later.
                        // We fail gracefully instead of crashing with FATAL_ERROR.
                        if (!m_auth_data) {
                                errorstream << "srp_user_new() failed for SRP auth: "
                                        << "memory allocation failure, auth_mech="
                                        << (int)chosen_auth_mechanism
                                        << std::endl;
                                m_chosen_auth_mech = AUTH_MECHANISM_NONE;
                                return;
                        }
                        ...
                        // v9.1 fix: Replace FATAL_ERROR_IF with graceful failure.
                        // FATAL_ERROR would crash the entire client. Instead, we
                        // log the error, clean up, and let the connection fail.
                        if (res != SRP_OK) {
                                errorstream << "srp_user_start_authentication() failed: "
                                        << "result=" << (int)res
                                        << ", auth_mech=" << (int)chosen_auth_mechanism
                                        << std::endl;
                                srp_user_delete((struct SRPUser *) m_auth_data);
                                m_auth_data = NULL;
                                m_chosen_auth_mech = AUTH_MECHANISM_NONE;
                        ...
                        if (!bytes_A || len_A == 0) {
                                errorstream << "srp_user_start_authentication() returned no bytes_A: "
                                        << "auth_mech=" << (int)chosen_auth_mechanism
                                        << std::endl;
                                srp_user_delete((struct SRPUser *) m_auth_data);
                                m_auth_data = NULL;
