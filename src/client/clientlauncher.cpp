                ...
                const Settings &cmd_args)
{
        ...
        if (!skip_main_menu) {
                ...
                // NOTE: Re-use existing structs (GameStartData). MainMenuData is
                // populated from GameStartData fields here, which is redundant. Ideally
                // MainMenuData would be eliminated or would wrap GameStartData instead
                // of duplicating its fields. This requires: (1) auditing which
                // MainMenuData fields are read after main_menu() returns that are NOT
                // in GameStartData (e.g. script_data fields), (2) adding those to
                // GameStartData or providing accessors, (3) refactoring main_menu()
                // to accept GameStartData directly.
