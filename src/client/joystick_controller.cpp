...
JoystickLayout create_default_layout()
{
        ...
        // NOTE: Button 0 has no mapping in this layout. On a PlayStation-style
        // controller, button 0 is typically D-pad Up or Square — it could be
        // mapped to a future KeyType if a suitable action is identified.
        ...
        // NOTE: Buttons 0, 1, 4, 5 have no mapping in this layout when the
        // start button modifier is also held. These correspond to face buttons
        // and triggers on a PlayStation-style controller. They could be mapped
        // to auxiliary actions (e.g., zoom, minimap, screenshot) if needed,
        // but adding too many modifier combos may hurt usability.
...
void JoystickController::onJoystickConnect(const std::vector<SJoystickInfo> &joystick_infos)
{
        ...
        if (id < 0 || id >= (s32)joystick_infos.size()) {
                // NOTE: No auto-detection of joystick ID yet. Defaulting to 0.
                // Auto-detection would iterate joystick_infos, prefer a controller
                // with axes/buttons (isController), and fall back to the first
                // non-empty name. This requires checking SDL_JoystickGetType()
                // or SJoystickInfo fields that may not be available in all builds.
                id = 0;
        }
