                ...
                const TileLayer &layer, u8 layernum, u32 numVertices)
{
        ...
        auto &index = buffer_index[layernum];
        ...
        if (it != index.end()) {
                ...
                if (p.vertices.size() + numVertices <= U16_MAX)
                        return p;
                // Buffer is full — create a spill buffer and update the index
                buffers.emplace_back(layer);
                it->second = buffers.size() - 1;
                return buffers.back();
        }

        // First occurrence of this layer — create new buffer and index it
        buffers.emplace_back(layer);
