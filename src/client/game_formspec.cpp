...
{
        // v9.46: Use Clay UI pause menu if the setting is enabled
        if (g_settings->getBool("clay_pause_menu") && m_clay_pause_menu) {
                m_clay_pause_menu->setVisible(true);
                m_clay_pause_menu->resetActions();
                // Mark game as paused (same as formspec pause menu)
                // The doPause flag is handled by the Game class
                return;
        }

        // Legacy formspec-based pause menu
...
bool GameFormSpec::handleCallbacks()
{
        ...
        // v9.46: Handle Clay pause menu actions
        if (m_clay_pause_menu && m_clay_pause_menu->isVisible()) {
                if (m_clay_pause_menu->wantsContinue()) {
                        m_clay_pause_menu->setVisible(false);
                        m_clay_pause_menu->resetActions();
                        // Close the pause state
                        g_gamecallback->disconnect_requested = false;
                }
                if (m_clay_pause_menu->wantsSettings()) {
                        m_clay_pause_menu->setVisible(false);
                        m_clay_pause_menu->resetActions();
                        g_gamecallback->openSettings();
                }
                if (m_clay_pause_menu->wantsSoundVolume()) {
                        m_clay_pause_menu->setVisible(false);
                        m_clay_pause_menu->resetActions();
                        g_gamecallback->changeVolume();
                }
                if (m_clay_pause_menu->wantsDisconnect()) {
                        m_clay_pause_menu->setVisible(false);
                        m_clay_pause_menu->resetActions();
                        g_gamecallback->disconnect();
                        return false;
                }
                if (m_clay_pause_menu->wantsExitToOS()) {
                        m_clay_pause_menu->setVisible(false);
                        m_clay_pause_menu->resetActions();
                        g_gamecallback->exitToOS();
#ifndef __ANDROID__
                ...
                if (m_clay_pause_menu->wantsChangePassword()) {
                        m_clay_pause_menu->setVisible(false);
                        m_clay_pause_menu->resetActions();
                        g_gamecallback->changePassword();
                }
