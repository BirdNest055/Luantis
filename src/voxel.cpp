...
void VoxelManipulator::addArea(const VoxelArea &area)
{
        ...
        // Allocate new data and clear flags using vectors for automatic cleanup
        std::vector<MapNode> new_data_vec(new_size);
        std::vector<u8> new_flags_vec(new_size);
        memset(new_flags_vec.data(), VOXELFLAG_NO_DATA, new_size);
        ...
        for(s32 y=m_area.MinEdge.Y; y<=m_area.MaxEdge.Y; y++)
        {
                ...
                memcpy(&new_data_vec[new_index], &m_data[old_index],
                                old_x_width * sizeof(MapNode));
                memcpy(&new_flags_vec[new_index], &m_flags[old_index],
        ...
        // Allocate persistent arrays (vectors used as temp for exception safety)
        m_data = new MapNode[new_size];
        memcpy(m_data, new_data_vec.data(), new_size * sizeof(MapNode));
        m_flags = new u8[new_size];
        memcpy(m_flags, new_flags_vec.data(), new_size);
