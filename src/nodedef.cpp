...
{
#if CHECK_CLIENT_BUILD()
        // Batch 35: Null out pointer after delete to prevent dangling pointer
        // if destructor is called twice or if reset() is called after destruction.
        delete visuals;
        visuals = nullptr;
#endif
}
        ...
        u8 connect_face) const
{
        ...
        if (f2.connect_sides > 0) {
                if ((f2.param_type_2 == CPT2_FACEDIR ||
                        ...
                        if (f2.param_type_2 == CPT2_FACEDIR ||
                                        f2.param_type_2 == CPT2_COLORED_FACEDIR) {
                                // Support arbitrary rotations (param2 & 0x1F) - Fix #29
                                // Previously only 4 rotations (param2 & 0x03) were supported
