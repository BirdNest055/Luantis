...
{
        // Clamp to safe range to prevent overflow in release builds where
        // the assert is not active. F1000_MIN/F1000_MAX are the largest
        // values guaranteed to produce in-range s32 results after multiplication.
        i = core::clamp(i, F1000_MIN, F1000_MAX);
