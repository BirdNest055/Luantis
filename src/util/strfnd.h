...
class BasicStrfnd {
	...
	String str;  // Owns the string data
	size_t pos;
public:
	BasicStrfnd(const String &s) : str(s), pos(0) {}
	BasicStrfnd(const T *ptr) : str(ptr), pos(0) {}
	BasicStrfnd(std::basic_string_view<T> sv) : str(sv.data(), sv.size()), pos(0) {}  // Copy from string_view
	// Note: Rvalue strings are copied (not dangling) since we own the data
