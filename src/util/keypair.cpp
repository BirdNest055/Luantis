...
std::vector<std::string> KeypairManager::listKeypairs() const
{
	...
	fs::GetDirEntries(m_keys_dir, entries, ".key");

	for (const auto &entry : entries) {
		// Strip the .key extension to get the username
		if (entry.size() > 4 && entry.substr(entry.size() - 4) == ".key") {
			result.push_back(entry.substr(0, entry.size() - 4));
		}
	}
