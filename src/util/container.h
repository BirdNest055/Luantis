...
protected:
        ...
        struct IterationHelper {
                ...
                ~IterationHelper() {
                        // Batch 34: RAII lock guard — explicit unlock before post,
                        // then m_lock destructor is a no-op (already unlocked)
                        m_lock.unlock();
        ...
        private:
                // Batch 34: RAII lock guard — replaces manual lock/unlock with
                // std::unique_lock, ensuring the mutex is always released even
                // if an exception is thrown during iteration
                IterationHelper(MutexedQueue<T> *parent) : q(parent), m_lock(parent->getMutex()) {
                }
