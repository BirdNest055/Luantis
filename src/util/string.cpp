...
std::wstring utf8_to_wide(std::string_view input)
{
        // Batch 28: Check for empty input to avoid allocating zero-size buffer
        if (input.empty())
                return L"";
        ...
        // Batch 28: Check MultiByteToWideChar return value for conversion failure
        int result = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, input.data(),
                input.size(), outbuf, outbuf_size);
        if (result == 0) {
                // Conversion failed; try again without strict validation
                memset(outbuf, 0, outbuf_size * sizeof(wchar_t));
...
std::string wide_to_utf8(std::wstring_view input)
{
        // Batch 28: Check for empty input
        if (input.empty())
                return "";
        ...
        // Batch 28: Check WideCharToMultiByte return value for conversion failure
        int result = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, input.data(),
                input.size(), outbuf, outbuf_size, NULL, NULL);
        if (result == 0) {
                // Conversion failed; try again without strict validation
                memset(outbuf, 0, outbuf_size);
...
bool is_valid_utf8(std::string_view str)
{
        ...
        while (i < str.size()) {
                ...
                if (c < 0x80) {
                        ...
                        if (i + 1 >= str.size())
                                return false;
                        if (!IS_UTF8_MULTB_INNER(str[i + 1]))
                                return false;
                        // Overlong check: must not encode value < 0x80
                        if ((c & 0x1E) == 0)
                                return false;
                        i += 2;
                } else if ((c & 0xF0) == 0xE0) {
                        ...
                        if (i + 2 >= str.size())
                                return false;
                        if (!IS_UTF8_MULTB_INNER(str[i + 1]) || !IS_UTF8_MULTB_INNER(str[i + 2]))
                                return false;
                        // Overlong check: must not encode value < 0x800
                        if ((c & 0x0F) == 0 && (static_cast<unsigned char>(str[i + 1]) & 0x20) == 0)
                                return false;
                        // Surrogate range check (U+D800..U+DFFF)
                        if (c == 0xED && (static_cast<unsigned char>(str[i + 1]) & 0xA0) == 0xA0)
                                return false;
                        i += 3;
                } else if ((c & 0xF8) == 0xF0) {
                        // 4-byte sequence
                        if (i + 3 >= str.size())
                                return false;
                        if (!IS_UTF8_MULTB_INNER(str[i + 1]) || !IS_UTF8_MULTB_INNER(str[i + 2]) ||
                                        !IS_UTF8_MULTB_INNER(str[i + 3]))
                                return false;
                        // Overlong check: must not encode value < 0x10000
                        if ((c & 0x07) == 0 && (static_cast<unsigned char>(str[i + 1]) & 0x30) == 0)
                                return false;
                        // Codepoint too large (> U+10FFFF)
                        if (c > 0xF4)
                                return false;
                        i += 4;
                } else {
                        // Invalid start byte (0x80-0xBF are continuation bytes, 0xF5-0xFF invalid)
                        return false;
                }
        }
        return true;
...
{
        // Batch 28: Guard against null pointer
        if (!str || str[0] == '\0')
                return 0;
...
bool parseColorString(const std::string &value, video::SColor &color, bool quiet,
                unsigned char default_alpha)
{
        // Batch 28: Guard against empty string access
        if (value.empty())
                return false;
...
std::string my_double_to_string(double number)
{
        if (std::isfinite(number)) {
                // Batch 28: Increased buffer from 64 to 128; "%.17g" can produce
                // up to ~25 chars but snprintf may need room for edge cases
                char buf[128];
