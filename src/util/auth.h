...
/// This is stored in AuthEntry::password alongside SRP verifiers (#1# format).
std::string encode_keypair_pubkey(const std::string &public_key);

/// Reads the DB-formatted keypair public key.
/// Returns true if the encoded string is a keypair entry (#2# format)
/// and the public key was successfully decoded.
