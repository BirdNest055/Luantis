...
public:
        ...
        // FLAG_INVALID: Set when loadBlock() encounters a corrupt block.
        // Invalid blocks should not be served to clients and should be
        // scheduled for re-generation by the emerge manager.
        static constexpr u8 FLAG_INVALID = 0x01;

        inline bool isInvalid() const { return m_flags & FLAG_INVALID; }
        inline void setFlag(u8 flag) { m_flags |= flag; }
        ...
        // Thread-safe variants: caller must hold the MapBlock's mutex
        inline MapNode getNode(s16 x, s16 y, s16 z, bool *valid_position) const
        {
        ...
        inline MapNode getNode(v3s16 p, bool *valid_position) const
        {
                return getNode(p.X, p.Y, p.Z, valid_position);
        }

        inline MapNode getNodeNoEx(v3s16 p) const
        {
                bool is_valid;
...
private:
        ...
        */
        ...
        // Flags for block state (e.g. FLAG_INVALID for corrupt blocks)
        u8 m_flags = 0;
