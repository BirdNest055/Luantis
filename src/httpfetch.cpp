        ...
        result(request_)
{
        ...
        case HTTP_GET:
                ...
                break;
        case HTTP_HEAD:
                // NOTE: HTTP HEAD responses discard the body (CURLOPT_NOBODY), but
                // response headers are also not returned to the caller. This makes
                // HEAD requests "kinda pointless" since the caller cannot inspect
                // Content-Length, ETag, Last-Modified, etc.
                // Root cause: HTTPFetchResult only stores: data, status_code, succeeded.
                // There is no field for response headers. The curl write callback
                // discards header data.
                // Proposed fix: Add a std::map<std::string, std::string> headers field
                // to HTTPFetchResult. Implement a curl header callback
                // (CURLOPT_HEADERFUNCTION) that parses headers into this map. For HEAD
                // requests, this would make them useful (e.g., checking file size before
                // a GET). For GET/POST, headers like Content-Type are also valuable.
                // The header callback should be set unconditionally, not just for HEAD.
