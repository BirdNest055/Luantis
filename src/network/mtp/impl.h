...
public:
        ...
        // v9: Set encryption state for a peer (called after SRP auth succeeds)
        void SetPeerEncryptionState(session_t peer_id, const PeerEncryptionState &state) override;

        // v9: Activate encryption for a peer via the send thread command queue.
        // This ensures that any previously queued packets (like AUTH_ACCEPT)
