...
void NetworkPacket::checkReadOffset(u32 from_offset, u32 field_size) const
{
        ...
        // Additional safety: if from_offset alone exceeds m_datasize, this would
        // also indicate a corrupted packet. Guard against integer overflow.
        if (from_offset > m_datasize) {
                std::ostringstream ss;
                ss << "Corrupted packet read offset: cmd=" << getCommand()
                        << " offset=" << from_offset
                        << " datasize=" << m_datasize;
                throw PacketError(ss.str());
        }
