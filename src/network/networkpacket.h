...
public:
        ...
        // Read a u16-length-prefixed binary string WITHOUT UTF-8 validation.
        // Use this for cryptographic keys, signatures, and other binary data
        // where the regular operator>> would corrupt non-UTF-8 bytes.
        std::string readBinaryString();

        // Read a u32-length-prefixed binary string WITHOUT UTF-8 validation.
        // Same as readBinaryString but with u32 length prefix (for putLongString).
        std::string readLongBinaryString();
