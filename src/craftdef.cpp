...
public:
        ...
        virtual void clear()
        {
                for (int type = 0; type <= craft_hash_type_max; ++type) {
                        ...
                        // Batch 35: Swap-and-release — frees hash map capacity
                        // immediately instead of keeping it allocated.
                        std::unordered_map<u64, std::vector<CraftDefinition*>> empty;
                        std::swap(m_craft_defs[type], empty);
                }
                // Batch 35: Swap-and-release for output definitions map
                std::unordered_map<std::string, std::vector<CraftDefinition*>> empty_out;
                std::swap(m_output_craft_definitions, empty_out);
        }
