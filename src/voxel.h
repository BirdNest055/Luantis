...
public:
        ...
        */
        ...
        u32 getVolume() const
        {
                // Use 64-bit arithmetic to avoid overflow during computation
                return (u32)((u64)m_cache_extent.X * (u64)m_cache_extent.Y * (u64)m_cache_extent.Z);
        }
...
public:
        ...
        */
        ...
        // Const version: only works if area is already loaded, throws otherwise
        MapNode getNode(const v3s16 &p) const
        {
                if (!m_area.contains(p))
                        throw InvalidPositionException(
                                "VoxelManipulator: const getNode: area not loaded");
                const s32 index = m_area.index(p);
                if (m_flags[index] & VOXELFLAG_NO_DATA)
                        throw InvalidPositionException(
                                "VoxelManipulator: const getNode: inexistent");
                return m_data[index];
        }
