...
std::vector<SubgameSpec> getAvailableGames()
{
        ...
        // NOTE: getAvailableGamePaths() is called N+1 times (once per gameid via
        // findSubgame() plus once in getAvailableGameIds()). It scans the filesystem
        // each time. To optimize: call getAvailableGamePaths() once here and pass
        // the GamePathMap to findSubgame() or inline the lookup. Alternatively,
        // refactor findSubgame() to accept an optional GamePathMap parameter, or
        // cache the result of getAvailableGamePaths() with a static variable that
        // invalidates when game paths change.
        return specs;
}
