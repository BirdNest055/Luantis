...
void PlayerDatabaseFiles::savePlayer(RemotePlayer *player)
{
        ...
        std::string savedir;
        savedir.reserve(m_savedir.size() + 1);
        savedir += m_savedir;
        savedir += DIR_DELIM;
        std::string path;
        path.reserve(savedir.size() + player->getName().size());
        path += savedir;
        path += player->getName();
        ...
        for (u32 i = 0; i < PLAYER_FILE_ALTERNATE_TRIES && !path_found; i++) {
                ...
                {
                        std::string tmp;
                        tmp.reserve(savedir.size() + player->getName().size() + itos(i).size());
                        tmp += savedir;
                        tmp += player->getName();
                        tmp += itos(i);
                        path = std::move(tmp);
...
bool PlayerDatabaseFiles::removePlayer(const std::string &name)
{
        ...
        players_path.reserve(m_savedir.size() + 1);
        players_path += m_savedir;
        players_path += DIR_DELIM;
        std::string path;
        path.reserve(players_path.size() + name.size());
        path += players_path;
        path += name;
        ...
        for (u32 i = 0; i < PLAYER_FILE_ALTERNATE_TRIES; i++) {
                ...
                {
                        std::string tmp;
                        tmp.reserve(players_path.size() + name.size() + itos(i).size());
                        tmp += players_path;
                        tmp += name;
                        tmp += itos(i);
                        path = std::move(tmp);
...
bool PlayerDatabaseFiles::loadPlayer(RemotePlayer *player, PlayerSAO *sao)
{
        ...
        players_path += DIR_DELIM;
        std::string path;
        path.reserve(players_path.size() + player->getName().size());
        path += players_path;
        path += player->getName();
        ...
        for (u32 i = 0; i < PLAYER_FILE_ALTERNATE_TRIES; i++) {
                ...
                {
                        std::string tmp;
                        tmp.reserve(players_path.size() + player_to_load.size() + itos(i).size());
                        tmp += players_path;
                        tmp += player_to_load;
                        tmp += itos(i);
                        path = std::move(tmp);
...
void PlayerDatabaseFiles::listPlayers(std::vector<std::string> &res)
{
        ...
        for (auto it = files.begin(); it != files.end(); ++it) {
                ...
                full_path.reserve(m_savedir.size() + 1 + filename.size());
                full_path += m_savedir;
                full_path += DIR_DELIM;
                full_path += filename;
...
{
        std::string path;
        path.reserve(m_savedir.size() + 1 + 8);
        path += m_savedir;
        path += DIR_DELIM;
        path += "auth.txt";
...
{
        m_storage_dir.reserve(savedir.size() + 1 + 11);
        m_storage_dir += savedir;
        m_storage_dir += DIR_DELIM;
        m_storage_dir += "mod_storage";
}
...
void ModStorageDatabaseFiles::endSave()
{
        ...
        for (auto it = m_modified.begin(); it != m_modified.end();) {
                ...
                {
                        std::string modpath;
                        modpath.reserve(m_storage_dir.size() + 1 + modname.size());
                        modpath += m_storage_dir;
                        modpath += DIR_DELIM;
                        modpath += modname;
                        if (!fs::safeWriteToFile(modpath, fastWriteJson(json))) {
...
Json::Value *ModStorageDatabaseFiles::getOrCreateJson(const std::string &modname)
{
        ...
        std::string path;
        path.reserve(m_storage_dir.size() + 1 + modname.size());
        path += m_storage_dir;
        path += DIR_DELIM;
        path += modname;
        if (fs::PathExists(path)) {
