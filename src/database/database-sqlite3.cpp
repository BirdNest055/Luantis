...
bool MapDatabaseSQLite3::saveBlock(const v3s16 &pos, std::string_view data)
{
        ...
        int res = sqlite3_step(m_stmt_write);
        if (res != SQLITE_DONE) {
                // Crash guard: log SQLite error instead of silently ignoring
                errorstream << "MapDatabaseSQLite3::saveBlock(): "
                        << "Failed to save block at (" << pos.X << "," << pos.Y << "," << pos.Z << "): "
                        << sqlite3_errmsg(m_database) << std::endl;
