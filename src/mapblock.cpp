...
void MapBlock::reallocate(u32 count, MapNode n)
{
        ...
        delete[] old_data;
        if (old_data && !m_is_mono_block && count == 1)
                porting::TrackFreedMemory(sizeof(MapNode) * nodecount);
}
...
{
        // For now monoblocks are disabled on the client.
        // NOTE: The client has known data races on the block's data. The mesh
        // generator thread reads block data (MapNode array) while the main thread
        // may modify it (e.g., client-side prediction, node placement). With mono-
        // blocks, the compressed representation is even more sensitive to torn reads.
        // Root cause: MapBlock::data is a raw pointer with no synchronization.
        // The mesh generator accesses it via MMVManip without holding the block's
        // mutex. On the server, blocks are only modified under envlock, but the
        // client's ClientMap has no equivalent protection.
        // Proposed fix: (1) Short-term: Keep monoblocks server-only (current behavior).
        // (2) Long-term: Add a shared_mutex to MapBlock. Mesh generation takes a
        // shared lock; modifications take an exclusive lock. Alternatively, snapshot
        // the block data into the MeshMakeData struct before spawning the mesh job.
