...
void MapBlock::reallocate(u32 count, MapNode n)
{
        ...
        // Use vector for exception-safe allocation, then release to raw pointer
        std::vector<MapNode> new_data_vec(count);
        std::fill_n(new_data_vec.data(), count, n);
        data = new MapNode[count];
        memcpy(data, new_data_vec.data(), count * sizeof(MapNode));
