...
bool setSystemPaths()
{
        ...
        if (len == 0) {
                ...
                // NOTE: Path still uses "Minetest" for backward compatibility on Windows.
                // When LUANTI_USER_PATH is not set, we fall back to %APPDATA%\Minetest.
                // Migration plan: change to DIR_DELIM "Luanti" and add fallback:
                //   1. If %APPDATA%\Luanti exists → use it
                //   2. Else if %APPDATA%\Minetest exists → copy/symlink to Luanti, use Luanti
                //   3. Else → create Luanti
                // Same migration applies to the Linux/macOS paths (see other setSystemPaths
