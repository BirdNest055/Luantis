...
void TileAnimationParams::calculateForParticle(float expirationtime)
{
	if (type == TAT_VERTICAL_FRAMES) {
		if (vertical_frames.length < 0) {
			// Negative length means "match particle lifetime"
			// Add small offset to prevent flicker on last frame
			vertical_frames.length = (expirationtime / -vertical_frames.length) + 0.1f;
		}
	} else if (type == TAT_SHEET_2D) {
		if (sheet_2d.frame_length < 0) {
			// Negative frame_length means "match particle lifetime"
			int frames = sheet_2d.frames_w * sheet_2d.frames_h;
			float runtime = (expirationtime / -sheet_2d.frame_length) + 0.1f;
			sheet_2d.frame_length = frames / runtime;
		}
	}
}
