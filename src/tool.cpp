...
PunchDamageResult getPunchDamage(
        ...
        bool do_hit = true;
        if (do_hit && punchitem) {
                // punch_operable allows punching regardless of the item in hand
                // Previously only allowed with empty hand, now applies to any non-tool item
                // NOTE: The punch_operable check only distinguishes between empty-hand
                // and non-empty-hand punching. It should also check whether the item is
                // a tool (via itemdef->tool_capabilities != nullptr) to allow punching
                // with non-tool items while still requiring punch_operable for bare hands.
                // Root cause: punchitem->name.empty() is used as a proxy for "no tool",
                // but a non-empty non-tool item (e.g., a block) is treated the same as
                // having a tool, which is incorrect.
                // Proposed fix: Resolve the item's ItemDefinition and check:
                //   bool is_tool = idef->get(punchitem->name).tool_capabilities != nullptr;
                // Then allow punching if: is_tool || punch_operable || punchitem->name.empty()
                // This requires passing IItemDefManager* into getPunchDamage().
                if (!itemgroup_get(armor_groups, "punch_operable") &&
                                punchitem->name.empty()) {
                        // Node doesn't have punch_operable and hand is empty - disallow
                        do_hit = false;
                }
                // If punch_operable is set, always allow punching (regardless of item)
        }
