...
void Environment::stepTimeOfDay(float dtime)
{
        ...
        // Batch 30: Clamp float-to-u32 conversion to avoid undefined behavior on overflow
        f32 units_f = m_time_conversion_skew * speed;
        u32 units = (units_f >= 0.f) ? ((units_f > (f32)U32_MAX) ? U32_MAX : (u32)units_f) : 0;
        bool sync_f = false;
        if (units > 0) {
                // Sync at overflow
