...
{
        // Batch 34: Double-checked locking — fast path: check cache without
        // lock first (safe because once inserted, cache entries are never removed
        // and the map is only appended to). If found, return immediately
        // without acquiring the mutex.
        {
                std::lock_guard<std::mutex> check_lock(cache_mutex);
                auto it = cache.find(d);
                if (it != cache.end())
                        return it->second;
        }

        // Slow path: generate under exclusive lock
