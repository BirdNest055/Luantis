...
public:
        ...
        // NOTE: This vector is only used by getIdsFromNrBacklog() to delimit groups
        // of names added via addNameIdList(). Each entry records how many names
        // were pushed in a single addNameIdList() call so that getIdsFromNrBacklog()
        // can pop the right number.
        // Root cause: The resolve queue is a flat vector of names with no group
        // delimiters. m_nnlistsizes stores group sizes so getIdsFromNrBacklog()
        // knows how many entries to consume per call.
        // Proposed replacement: Replace both m_nodenames and m_nnlistsizes with a
        // std::vector<std::vector<std::string>> (list-of-groups). Each inner
        // vector is one addNameIdList() batch. getIdsFromNrBacklog() would pop
        // one inner vector instead of counting with m_nnlistsizes. This removes
        // the need for the parallel size vector entirely.
        // Migration: Update addNameIdList(), getIdsFromNrBacklog(), and
        // nodeResolveInternal() to use the nested vector. The m_nodenames_idx
        // counter would also be replaced by an outer-vector iterator.
