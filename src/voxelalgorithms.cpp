        ...
        std::map<v3s16, MapBlock*> &modified_blocks)
{
        ...
        while (from_nodes.next(current_light, current)) {
                ...
                // brighter neighbor.
                // Batch 37: Clamp light_source + 1 to prevent overflow beyond LIGHT_SUN
                u8 brightest_neighbor_light = std::min(static_cast<u8>(f.light_source + 1), static_cast<u8>(LIGHT_SUN));
                        ...
                        if (new_light < old_light) {
                                ...
                                if (bank == LIGHTBANK_DAY && old_light == LIGHT_SUN) {
                                        for (s16 y = p.Y - 1;; y--) {
                                                ...
                                                MapBlock *block2 = map->getBlockNoCreateNoEx(
                                                        block_pos2);
                                                // Batch 37: Don't push null block into unlight queue
                                                if (!block2)
                                                        break;
                                ...
                                if (bank == LIGHTBANK_DAY && new_light == LIGHT_SUN) {
                                        for (s16 y = p.Y - 1;; y--) {
                                                ...
                                                MapBlock *block2 = map->getBlockNoCreateNoEx(
                                                        block_pos2);
                                                // Batch 37: Don't push null block into relight queue
                                                if (!block2)
                                                        break;
