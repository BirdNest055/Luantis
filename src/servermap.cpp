...
bool MapDatabaseAccessor::saveBlock(v3s16 blockpos, const std::string &data)
{
        MutexAutoLock lock(mutex);
        return dbase->saveBlock(blockpos, data);
}

bool MapDatabaseAccessor::deleteBlock(v3s16 blockpos)
{
        MutexAutoLock lock(mutex);
        return dbase->deleteBlock(blockpos);
}
...
bool ServerMap::saveBlock(MapBlock *block)
{
        ...
        // Save using thread-safe wrapper
        bool ret = m_db.saveBlock(p3d, serialized);
        if (ret) {
                block->resetModified();
        }
        return ret;
...
bool ServerMap::saveBlock(MapBlock *block, MapDatabase *db, int compression_level)
{
        ...
        */
        ...
        // NOTE: Zero-copy serialization possible with C++20 or custom rdbuf.
        // Root cause: block->serialize() writes to std::ostringstream, then
        // o.str() copies the entire buffer to a std::string for db->saveBlock().
        // This double-copy (stream buffer -> string -> database) is wasteful for
        // large map blocks.
        // Proposed alternatives:
        //   (1) C++26 std::ispanstream: serialize into a pre-allocated buffer
        //       without intermediate string copy.
        //   (2) Custom std::streambuf that writes directly into a std::vector<u8>
        //       or std::string via overflow()/xsputn() overrides, then move the
        //       container into saveBlock().
        //   (3) Add a MapBlock::serializeToBuffer(std::vector<u8>&) overload
        //       that bypasses iostream entirely, writing the version byte and
        //       block data directly into the vector.
        // Option (3) is the most practical and can be done with C++17 today.
