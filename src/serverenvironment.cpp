...
void ServerEnvironment::step(float dtime)
{
        ...
        if (m_active_blocks_mgmt_interval.step(dtime, m_cache_active_block_mgmt_interval / m_fast_active_block_divider)) {
                ...
                */
                ...
                for (const v3s16 &p: blocks_added) {
                        ...
                        if (!block) {
                                // NOTE: Blocks that fail to emerge here are removed from
                                // m_active_blocks and will only be reconsidered on the next
                                // ActiveBlockList cycle (every m_cache_abm_interval seconds).
                                // This means objects in those blocks experience activation
                                // latency of one full cycle.
                                // Root cause: getBlockOrEmerge() may trigger an async emerge
                                // but returns nullptr immediately if the block isn't loaded yet.
                                // Proposed fix: Add a set<v3s16> m_pending_activate_blocks that
                                // records positions where activation failed. In onMapEdit()
                                // or the emerge callback, check if the newly loaded block is
                                // in m_pending_activate_blocks and call activateBlock() on it.
                                // Clear the entry after successful activation.
        ...
        */
        if (m_active_blocks_nodemetadata_interval.step(dtime, m_cache_nodetimer_interval)) {
                ...
                // Note: Blocks may have been activated recently, but since the
                // interval is small, running timers on newly activated blocks
