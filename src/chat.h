...
private:
        ...
        // Lazy-reformatting optimization: When true, indicates that the
        // formatted lines are stale and need reformatting on the next
        // access. This avoids reformatting ALL lines (including invisible
        // ones) on every resize; instead, only visible lines are reformatted
        // on demand. Set to true when m_cols changes or lines are added/removed.
        bool m_needs_reformat = false;
};
