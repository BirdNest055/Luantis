...
void set_default_settings()
{
...
#endif
        settings->setDefault("show_security_overlay", "true");
        // v9: Security overlay display mode. Options: "mini", "standard", "detailed", "off"
        settings->setDefault("security_overlay_mode", "standard");
        settings->setDefault("show_connection_info", "false");
        // v9: Encryption detail overlays — toggleable in Settings, update every second
        // Each shows specific live encryption data when enabled
        settings->setDefault("show_enc_session_overlay", "false");
        settings->setDefault("show_enc_packets_overlay", "false");
        settings->setDefault("show_enc_cipher_overlay", "false");
        settings->setDefault("show_enc_score_overlay", "false");
        settings->setDefault("show_enc_authfail_overlay", "false");
        settings->setDefault("show_enc_nonce_overlay", "false");
        settings->setDefault("show_enc_latency_overlay", "false");
        settings->setDefault("show_enc_timeline_overlay", "false");
        // v9.1: New encryption detail overlays for ECDH PFS, trust, health, bandwidth
        settings->setDefault("show_enc_pfs_overlay", "false");
        settings->setDefault("show_enc_trust_overlay", "false");
        settings->setDefault("show_enc_health_overlay", "false");
        settings->setDefault("show_enc_bandwidth_overlay", "false");

        // Security info runtime settings (populated by client when connected)
        // These are not saved to disk — they represent the current connection state
        settings->setDefault("security_info_state", "Not Connected");
        settings->setDefault("security_info_encryption", "N/A");
        settings->setDefault("security_info_key_exchange", "N/A");
        settings->setDefault("security_info_authentication", "N/A");
        settings->setDefault("security_info_cipher_suite", "N/A");
        settings->setDefault("security_info_cert_status", "N/A");
        settings->setDefault("security_info_forward_secrecy", "N/A");
        settings->setDefault("security_info_replay_protection", "N/A");
        settings->setDefault("security_info_protocol_version", "N/A");
        settings->setDefault("security_info_server_address", "N/A");
        settings->setDefault("security_info_server_port", "N/A");
        // v8: Security Info Tab — additional runtime settings
        settings->setDefault("security_info_session_id", "N/A");
        ...
        settings->setDefault("toggle_aux1_key", "true");
