...
const ObjDef *ObjDefManager::get(ObjDefHandle handle) const
{
        u32 index = validateHandle(handle);
        return (index != OBJDEF_INVALID_INDEX) ? getRaw(index) : NULL;
}

ObjDef *ObjDefManager::get(ObjDefHandle handle)
{
        return const_cast<ObjDef*>(
                static_cast<const ObjDefManager*>(this)->get(handle));
}
...
const ObjDef *ObjDefManager::getRaw(u32 index) const
{
        return m_objects[index];
}

ObjDef *ObjDefManager::getRaw(u32 index)
{
        return const_cast<ObjDef*>(
                static_cast<const ObjDefManager*>(this)->getRaw(index));
}
...
const ObjDef *ObjDefManager::getByName(const std::string &name) const
{
        for (size_t i = 0; i != m_objects.size(); i++) {
                const ObjDef *obj = m_objects[i];
...
ObjDef *ObjDefManager::getByName(const std::string &name)
{
        return const_cast<ObjDef*>(
                static_cast<const ObjDefManager*>(this)->getByName(name));
}
