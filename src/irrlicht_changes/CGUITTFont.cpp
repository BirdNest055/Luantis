...
CGUITTGlyphPage* CGUITTFont::createGlyphPage(const u8 pixel_mode)
{
        ...
        // Batch 30: Guard against division by zero if size is 0
        if (size <= 0) {
                errorstream << "CGUITTGlyphPage: invalid glyph size" << std::endl;
                delete page;
                return 0;
        }
...
std::u32string CGUITTFont::convertWCharToU32String(const wchar_t* const charArray) const
{
        ...
        while (*p) {
                ...
                if (c >= 0xD800 && c < 0xDC00) {
                        ...
                        // Batch 28: Validate decoded codepoint is in valid Unicode range
                        char32_t decoded = 0x10000 + ( ((c & 0x3ff) << 10) | (c2 & 0x3ff) );
                        if (decoded > 0x10FFFF) {
                                ret.push_back(0xFFFD); // replacement character
                        } else {
