        ...
        if (font && BrokenText.size()) {
                ...
                for (const EnrichedString &str : BrokenText) {
                        ...
                        } else
                        {
                                ...
                                font->draw(str.c_str(),
                                        r, str.getDefaultColor(),
                                        // NOTE: Colorization not implemented for native Irrlicht fonts.
                                        // Root cause: IGUIFont::draw() only accepts a single SColor — it
                                        // has no per-character color API. CGUITTFont (EGFT_CUSTOM) supports
                                        // EnrichedString natively, but the built-in font does not.
                                        // Proposed solution: iterate over EnrichedString substrings by color
                                        // range, calling font->draw() for each segment with its color and
                                        // adjusted X offset. Requires measuring each segment's pixel width.
