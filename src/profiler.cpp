...
{
        // Batch 34: Double-checked locking — fast path: avoid acquiring mutex
        // when the profiler is not enabled (g_profiler is null or disabled)
        if (!g_profiler)
                return;
...
{
        // Batch 34: Double-checked locking — skip if profiler not active
        if (!g_profiler)
                return;
...
{
        // Batch 34: Race condition guard — add lock to protect m_data reads
        MutexAutoLock lock(m_mutex);
        auto it = m_data.find(name);
        if (it == m_data.end())
                return 0;
...
{
        // Batch 34: Race condition guard — add lock to protect m_start_time read
        MutexAutoLock lock(m_mutex);
...
{
        // Batch 34: Mutex scope reduction — copy data under lock, then
        // format and print outside the lock to reduce contention.
        GraphValues values;
