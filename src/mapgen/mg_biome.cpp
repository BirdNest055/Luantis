...
biome_t *BiomeGenOriginal::getBiomes(s16 *heightmap, v3s16 pmin)
{
        ...
        for (s16 xr = 0; xr < m_csize.X; xr++) {
                ...
                // Crash guard: calcBiomeFromNoise can return null if m_bmgr is null
                biomemap[i] = biome ? biome->index : BIOME_NONE;
        }
...
{
        // Crash guard: m_bmgr can be null if biome manager was not initialized
        if (!m_bmgr)
                return nullptr;
