...
ObjDef *DecoSchematic::clone() const
{
        ...
        // We do not own this schematic, yet we only have a pointer to it
        // and not a handle. We are left with no option but to clone it ourselves.
        // This is a waste of memory. A future refactor should replace raw pointers
        // with ObjDefHandle (index into the global ObjDef tree) so that cloning
        // shares the reference instead of duplicating the schematic.
        FATAL_ERROR_IF(!schematic, "DecoSchematic::clone(): null schematic pointer");
