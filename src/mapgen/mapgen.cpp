        ...
        gennotify(emerge->createNotifier())
{
        ...
        */
        ...
        // Null-pointer check: some mapgens might be called before node
        // definitions are fully loaded
        FATAL_ERROR_IF(!ndef, "Mapgen: NodeDefManager (ndef) is null — "
                "mapgen created before node definitions are loaded");
}
        ...
        for (int z = a.MinEdge.Z; z <= a.MaxEdge.Z; z++) {
                for (int y = a.MinEdge.Y; y <= a.MaxEdge.Y; y++) {
                        ...
                        for (int x = a.MinEdge.X; x <= a.MaxEdge.X; x++, i++) {
                                ...
                                // used here for lighting propagation. This is now abstracted
                                // with the getLightRaw()/setLightRaw() inline helpers defined
                                // in mapnode.h, which avoid the overhead of MapNode::get/setLight()
                                // (which allocate LightPair). param1 packs both day and night
                                // light into one byte (upper nibble = day, lower nibble = night).
                                // The inline helpers provide type-safe per-bank access suitable
                                // for hot loops like lighting propagation.
