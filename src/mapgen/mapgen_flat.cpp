...
s16 MapgenFlat::generateTerrain()
{
        ...
        // Validate ground_level: it must be within map generation bounds
        s16 validated_ground_level = rangelim(ground_level,
        ...
        for (s16 x = node_min.X; x <= node_max.X; x++, ni2d++) {
                s16 stone_level = validated_ground_level;
                float n_terrain = use_noise ? noise_terrain->result[ni2d] : 0.0f;

                if ((spflags & MGFLAT_LAKES) && n_terrain < lake_threshold) {
                        s16 depress = (lake_threshold - n_terrain) * lake_steepness;
                        stone_level = validated_ground_level - depress;
                } else if ((spflags & MGFLAT_HILLS) && n_terrain > hill_threshold) {
                        s16 rise = (n_terrain - hill_threshold) * hill_steepness;
                        stone_level = validated_ground_level + rise;
                }
