...
int MapgenValleys::generateTerrain()
{
        ...
        for (s16 x = node_min.X; x <= node_max.X; x++, index_2d++) {
                ...
                // NaN check: valley depth noise can produce NaN which would
                // propagate through the terrain calculations causing corrupt terrain
                if (std::isnan(n_valley) || std::isnan(n_valley_profile)) {
                        index_2d++;
                        continue;
                }
