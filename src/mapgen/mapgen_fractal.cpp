        ...
        for (s16 z = node_min.Z; z <= node_max.Z; z++) {
                for (s16 y = node_min.Y - 1; y <= node_max.Y + 1; y++) {
                        ...
                        for (s16 x = node_min.X; x <= node_max.X; x++, vi++, index2d++) {
                                ...
                                // NaN check: fractal formulas and seabed noise can produce
                                // NaN for certain parameter combinations; skip such positions
                                if (noise_seabed && std::isnan(noise_seabed->result[index2d]))
                                        continue;
