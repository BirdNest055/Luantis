...
// DEPRECATED: nparams_caveliquids is the old hardcoded cave liquid noise.
// Cave liquids are now defined and located using biome definitions (see
// use_biome_liquid in CavesRandomWalk). This static exists only as a
// fallback for mapgens that don't provide biome liquid data. Removal plan:
//   1. After a deprecation period, remove np_caveliquids from MapgenParams
//   2. Make use_biome_liquid=true the only path (remove the else branch
//      in CavesRandomWalk::makeCave() around line 530)
//   3. Delete this static and the NoiseParams* member in cavegen.h
...
void CavesRandomWalk::carveRoute(v3f vec, float f, bool randomize_xz)
{
        ...
        if (flooded) {
                if (use_biome_liquid) {
                        ...
                        // DEPRECATED: 'np_caveliquids' noise is deprecated. Cave liquids are now
