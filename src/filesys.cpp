                ...
                std::string *removed, int count)
{
        ...
        for (int i = 0; i < count; ++i) {
                ...
                // On Windows, don't strip delimiter after drive letter (e.g., "C:")
                while (remaining > 1 && IsDirDelimiter(path[remaining-1])) {
                        // Check for Windows drive letter pattern (e.g., "C:\")
                        // Don't strip if remaining would leave just "C:"
                        #if defined(_WIN32) || defined(_WIN64)
                        if (remaining == 3 && path.size() >= 2 && path[1] == ':')
                                break;
                        // Also check for drive-letter-only paths like "C:" (2 chars)
                        // If the path is just a drive letter, don't strip anything
                        if (remaining == 2 && path.size() >= 2 && path[1] == ':')
                                break;
                        #endif
                        remaining--;
                }
