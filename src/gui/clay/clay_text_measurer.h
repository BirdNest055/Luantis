...
public:
	...
	static uintptr_t encodeFontSpec(uint32_t size, uint32_t mode = 0, bool bold = false, bool italic = false);

	/// Decode font parameters from a uintptr_t
	static void decodeFontSpec(uintptr_t encoded, uint32_t &size, uint32_t &mode, bool &bold, bool &italic);
