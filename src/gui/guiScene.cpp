        ...
        : IGUIElement(gui::EGUIET_ELEMENT, env, parent, id, rect)
{
        ...
        m_cam->setFOV(GUITheme::Sizing::SCENE_CAMERA_FOV * core::DEGTORAD);
}
...
void GUIScene::draw()
{
        ...
        // Continuous rotation
        if (m_inf_rot)
                rotateCamera(v3f(0.f, -GUITheme::Sizing::SCENE_ROTATION_SPEED * (float)dtime_ms, 0.f));
...
inline void GUIScene::calcOptimalDistance()
{
        ...
        if (zoomX < zoomY)
                dist = (max_width / (far_width / cam_far)) + (GUITheme::Sizing::SCENE_CAMERA_DISTANCE_MULT * max_width);
        else
                dist = (height / (far_height / cam_far)) + (GUITheme::Sizing::SCENE_CAMERA_DISTANCE_MULT * max_width);
...
bool GUIScene::correctBounds(v3f &rot)
{
        ...
        if (rot.X < 90.f) {
                if (rot.X > GUITheme::Sizing::SCENE_ROTATION_MAX_1) {
                        rot.X = GUITheme::Sizing::SCENE_ROTATION_MAX_1;
                        return true;
                }
        } else if (rot.X < GUITheme::Sizing::SCENE_ROTATION_MAX_2) {
                rot.X = GUITheme::Sizing::SCENE_ROTATION_MAX_2;
                return true;
        }
