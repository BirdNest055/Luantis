                ...
                const v3s16 &rotation_speed)
{
        ...
        if (has_mesh) {
                ...
                if (inventory_texture) {
                        ...
                        color = GUITheme::Colors::ITEM_NO_TEXTURE;
        ...
        if (def.type == ITEM_TOOL && item.wear != 0) {
                // Draw a progressbar
                float barheight = static_cast<float>(rect.getHeight()) / GUITheme::Sizing::WEAR_BAR_DIVISOR;
                float barpad_x = static_cast<float>(rect.getWidth()) / GUITheme::Sizing::WEAR_BAR_DIVISOR;
                float barpad_y = static_cast<float>(rect.getHeight()) / GUITheme::Sizing::WEAR_BAR_DIVISOR;
                ...
                color = GUITheme::Colors::ITEM_WEAR_BG;
        ...
        if (font != nullptr && (item.count >= 2 || !count_text.empty())) {
                ...
                video::SColor color = GUITheme::Colors::ITEM_COUNT_TEXT;
                ...
                ItemRotationKind rotation_kind)
{
        drawItemStack(driver, font, item, rect, clip, client, rotation_kind,
                v3s16(0, 0, 0), v3s16(0, GUITheme::Sizing::ITEM_RENDER_ROTATION_Y, 0));
}
