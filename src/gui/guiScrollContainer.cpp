...
void GUIScrollContainer::setScrollBar(GUIScrollBar *scrollbar)
{
        ...
        if (m_scrollbar && m_content_padding_px.has_value() && m_scrollfactor != 0.0f) {
                ...
                // Batch 32: guard against division by zero for visible_content_px
                s32 page_size = (visible_content_px > 0)
                        ? (total_content_px * scrollbar_px) / visible_content_px
                        : scrollbar_px;
                m_scrollbar->setPageSize(page_size);
        }
}
