...
void GUIButton::setFromStyle(const StyleSpec& style)
{
        ...
        if (style.isNotDefault(StyleSpec::BGCOLOR)) {
                ...
                if (!Styles[style.getState()].isNotDefault(StyleSpec::BGCOLOR)) {
                                if (pressed) {
                                        BgColor = multiplyColorValue(BgColor, GUITheme::ButtonModifiers::PRESS_DARKEN);

                                        for (size_t i = 0; i < 4; i++)
                                                Colors[i] = multiplyColorValue(Colors[i], GUITheme::ButtonModifiers::PRESS_DARKEN);
                                } else if (hovered) {
                                        BgColor = multiplyColorValue(BgColor, GUITheme::ButtonModifiers::HOVER_BRIGHTEN);

                                        for (size_t i = 0; i < 4; i++)
                                                Colors[i] = multiplyColorValue(Colors[i], GUITheme::ButtonModifiers::HOVER_BRIGHTEN);
                                }
                }

        } else {
                BgColor = GUITheme::Colors::BUTTON_BG_DEFAULT;
                for (size_t i = 0; i < 4; i++) {
                        video::SColor base =
                                        Environment->getSkin()->getColor((gui::EGUI_DEFAULT_COLOR)i);
                        if (pressed) {
                                Colors[i] = multiplyColorValue(base, GUITheme::ButtonModifiers::PRESS_DARKEN);
                        } else if (hovered) {
                                Colors[i] = multiplyColorValue(base, GUITheme::ButtonModifiers::HOVER_BRIGHTEN);
                        } else {
                                Colors[i] = base;
                        }
                }
        ...
        if (style.isNotDefault(StyleSpec::TEXTCOLOR)) {
                ...
                setOverrideColor(GUITheme::Colors::BUTTON_TEXT_DEFAULT);
