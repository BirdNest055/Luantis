        ...
        m_tsrc(tsrc)
{
        ...
        if (m_font) {
                m_font->grab();
                m_rowheight = m_font->getDimension(L"Ay").Height + GUITheme::Sizing::TABLE_ROW_PADDING;
                ...
                std::vector<std::string> &content)
{
        ...
        if (ttfont) {
                ...
                desired_image_scale = std::max(1.0f, ttfont->getFontSize() / GUITheme::Fonts::FONT_SCALE_DIVISOR);
        }

        // Get em width. Pedantically speaking, the width of "M" is not
        // necessarily the same as the em width, but whatever, close enough.
        s32 em = GUITheme::Sizing::TABLE_EM_FALLBACK;
        if (m_font)
        ...
        for (s32 j = 0; j < colcount; ++j) {
                ...
                s32 padding = myround(GUITheme::Sizing::TABLE_COL_PADDING_EM * em);
                ...
                if (columntype == COLUMN_TYPE_INDENT ||
                                columntype == COLUMN_TYPE_TREE) {
                        width = myround(em * GUITheme::Sizing::TABLE_INDENT_EM); // default indent width
                }
                ...
                if (columntype == COLUMN_TYPE_TEXT) {
                        ...
                        for (s32 i = 0; i < rowcount; ++i) {
                                video::SColor cellcolor = GUITheme::Colors::TABLE_TEXT_DEFAULT;
...
bool GUITable::OnEvent(const SEvent &event)
{
        ...
        if (event.EventType == EET_KEY_INPUT_EVENT) {
                ...
                if (event.KeyInput.PressedDown && (
                        ...
                        if (now - m_keynav_time >= GUITheme::Timing::TABLE_KEYNAV_TIMEOUT_MS)
                                m_keynav_buffer = L"";
                        m_keynav_time = now;
        ...
        if (event.EventType == EET_MOUSE_INPUT_EVENT) {
                ...
                if (event.MouseInput.Event == EMIE_MOUSE_WHEEL) {
                        m_scrollbar->setPosInterpolated(m_scrollbar->getTargetPos() +
                                        (event.MouseInput.Wheel < 0 ? -(s32)GUITheme::Timing::MOUSE_WHEEL_SCROLL_MULTIPLIER : (s32)GUITheme::Timing::MOUSE_WHEEL_SCROLL_MULTIPLIER) *
                                        - (s32) m_rowheight / 2);
                        return true;
                }
