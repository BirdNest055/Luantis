...
public:
        ...
        const core::aabbox3d<f32> &getBoundingBox() const override {
                // NOTE: IMesh::getBoundingBox() is pure virtual, forcing every mesh subclass
                // to implement it. For SkinnedMesh the bounding box depends on the current
                // animation frame, but the base interface has no way to pass that context.
                // The returned StaticPoseBox is therefore a conservative approximation
                // (static pose bounds). To make this context-aware, IMesh::getBoundingBox()
                // would need an overload accepting animation state, or the scene node would
                // need to call a SkinnedMesh-specific method instead.
        ...
        struct SJoint
        {
                ...
                // NOTE: AttachedMeshes lists mesh buffer indices that belong to this joint.
                // Currently this is a separate vector, but it could be refactored into an
                // optional field on the mesh buffer itself (e.g., optional<u32> ParentJointID
                // on each SkinnedMeshBuffer). This would eliminate the O(joints × meshes)
                // lookup in animateJoints() and make the parent relationship discoverable
                // from the mesh buffer side. Blocked on: SkinnedMeshBuffer currently has no
                // knowledge of its parent joint, so adding the field requires changes to
                // SkinnedMesh::animateJoints(), all loaders, and the serialization path.
                ...
                u16 JointID; // NOTE: Ideally this would be a simple index, and parent-child relationships
                // would use only IDs (ParentJointID, already optional<u16>). However, the .x loader
                // (CXMeshFileLoader) abuses SJoint by storing raw pointers during the loading phase
                // before IDs are assigned, making a pure-ID approach require a two-pass refactor:
                //   Pass 1: Parse hierarchy and assign JointIDs.
                //   Pass 2: Resolve all pointer references to JointIDs.
                // The .gltf and .b3d loaders already use IDs exclusively, so this refactoring only
                // needs to target CXMeshFileLoader. After that, JointID could become a simple index
                // and pointer-based parent lookups could be removed entirely.
...
public:
        ...
        // NOTE: The .x and .b3d loaders bypass the SkinnedMeshBuilder abstraction — they
        // directly manipulate SkinnedMesh internals (joints, keyframes, mesh buffers) instead
        // of going through a separate "extractor" class that is used and destroyed within the
        // loading process (contrast with the .gltf mesh loader, which uses a proper
        // SkinnedMeshBuilder). This means we need an empty default constructor here as a
        // workaround. Proper fix: refactor the .x and .b3d loaders to use SkinnedMeshBuilder
        // for all mesh construction, eliminating the need for direct SkinnedMesh access.
        SkinnedMeshBuilder() {}
