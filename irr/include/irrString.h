...
/* NOTE: These UTF-8 conversion functions are imported from Luanti's util/string.h
 * rather than implemented locally in Irrlicht. This avoids code duplication since
 * Irrlicht's string class needs wide↔UTF-8 conversion but the canonical implementations
 * live in the engine's utility library. If Irrlicht is ever used standalone (without
 * Luanti), these externs would need to be replaced with local implementations or
 * the functions moved into irr::core. */
