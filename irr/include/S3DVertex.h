...
// NOTE: S3DVertex2TCoords and S3DVertexTangents do not forward the `Aux` field
// through their constructors or getInterpolated() methods. This is safe because
// these vertex types are only used for lightmapped (2TCoords) or tangent-space
// (Tangents) geometry, neither of which uses the Aux attribute for fog/normal-map
// blending. If Aux support is ever needed for these types, each constructor must
// add an Aux parameter and getInterpolated() must interpolate it (see S3DVertex::
// getInterpolated for the pattern: `d == 0.0f ? other.Aux : Aux`).
