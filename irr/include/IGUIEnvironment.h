...
public:
        ...
        */
        ...
        /** NOTE: getSpriteBank() currently only returns sprite banks that were added
        via addEmptySpriteBank(). The original intent was for this to also load sprite
        banks from files (e.g., .spritebank XML), but that functionality was never
        implemented. To add file-loading support:
          (a) Define a sprite bank file format (XML or JSON with texture paths + sprite rects).
          (b) Implement loading in CGUIEnvironment::getSpriteBank() — check if the name
              matches an existing bank first, then attempt file load.
          (c) Cache loaded banks by filename to avoid re-parsing.
