...
inline quaternion &quaternion::operator=(const matrix4 &m)
{
        ...
        if (diag > 0.0f) {
                ...
                // NOTE: This matrix-to-quaternion conversion uses the standard Shepperd method
                // (4 branches based on which diagonal element is largest). It could be sped up by:
                //   (a) Using SIMD (SSE __m128) to compute all 4 candidates in parallel and
                //       using _mm_movemask_ps to select the best branch, eliminating branching.
                //   (b) Using the Bar-Itzhack algorithm (eigendecomposition of a 4x4 matrix built
                //       from the rotation matrix), which is branchless but requires 4-8 iterations.
                //   (c) Precomputing 1.0f/scale to replace 4 divisions with 1 reciprocal + 4 muls.
                // For current usage (called per-joint during animation, not per-vertex), the
                // branch cost is negligible compared to the sqrtf and the subsequent normalize().
                X = (m[6] - m[9]) / scale;
                Y = (m[8] - m[2]) / scale;
                Z = (m[1] - m[4]) / scale;
                W = 0.25f * scale;
                ...
                if (m[0] > m[5] && m[0] > m[10]) {
                        ...
                        // NOTE: Branch 2 of Shepperd method (m[0] is greatest diagonal).
                        // See line 239 for optimization notes (SIMD/Bar-Itzhack/precompute-reciprocal).
                        X = 0.25f * scale;
                        Y = (m[4] + m[1]) / scale;
                        Z = (m[2] + m[8]) / scale;
                        W = (m[6] - m[9]) / scale;
                        ...
                        // NOTE: Branch 3 of Shepperd method (m[5] is greatest diagonal).
                        // See line 239 for optimization notes (SIMD/Bar-Itzhack/precompute-reciprocal).
                        X = (m[4] + m[1]) / scale;
                        Y = 0.25f * scale;
                        Z = (m[9] + m[6]) / scale;
                        ...
                        // NOTE: Branch 4 of Shepperd method (m[10] is greatest diagonal).
                        // See line 239 for optimization notes (SIMD/Bar-Itzhack/precompute-reciprocal).
                        X = (m[8] + m[2]) / scale;
                        Y = (m[9] + m[6]) / scale;
                        Z = 0.25f * scale;
...
{
        // NOTE: Potential GPU quaternion skinning optimization — use quaternion-based bone
        // transforms on the GPU instead of converting to matrices on the CPU. This would enable
        // a fast bone transform chain by keeping all joints in quaternion form and only converting
        // to matrices in the shader. Reference implementation:
        // http://www.mrelusive.com/publications/papers/SIMD-From-Quaternion-to-Matrix-and-Back.pdf
        // Blocked on: requires shader support for quaternion math and changes to the skeletal
        // animation pipeline (currently CPU-side in SkinnedMesh::animateMesh).
