...
void AnimatedMeshSceneNode::OnAnimate(u32 timeMs)
{
        ...
        // NOTE: copyOldTransforms() is always called here even when there are no bone
        // overrides or animation blending, making it unnecessary in that case. To optimize,
        // add a bool HasBoneOverrides flag (set when any bone override is applied via
        // setJointMode/setTransitionTime), and guard this call:
        //   if (HasBoneOverrides || Mesh->getJointCount() > 0) copyOldTransforms();
        // The cost of the unnecessary call is O(jointCount) pointer copies, which is
        // typically <1μs for models with <100 joints.
        copyOldTransforms();
