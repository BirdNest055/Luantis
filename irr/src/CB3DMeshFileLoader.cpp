        ...
        while ((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) // this chunk repeats
        {
                ...
                if (B3dMaterial.Textures[1]) {
                        ...
                        if (B3dMaterial.Textures[0]->Flags & 0x2) { // (Alpha mapped)
                                ...
                                B3dMaterial.Material.MaterialType = video::EMT_TRANSPARENT_ALPHA_CHANNEL_REF; // NOTE: B3D format flag 0x4 indicates a masked (color-key) texture.
                                // Ideally, pixels matching the color key should be made transparent by creating
                                // a modified texture with alpha=0 for key-colored pixels, then using
                                // EMT_TRANSPARENT_ALPHA_CHANNEL for smooth edges. The current approach uses
                                // EMT_TRANSPARENT_ALPHA_CHANNEL_REF which performs hard alpha testing (1-bit),
                                // producing jagged edges on color-key boundaries. To fix properly:
                                //   (a) After loading the texture, iterate over its pixels and set alpha=0
                                //       where the pixel color matches the B3D color key (typically pure magenta
                                //       #FF00FF or as specified in the B3D flags/texture data).
                                //   (b) Switch material type to EMT_TRANSPARENT_ALPHA_CHANNEL for soft blending.
                                //   (c) This requires access to the loaded ITexture* and its pixel data at load
                                //       time, which may not be available until the video driver processes it.
