...
SColor CImage::getPixel(u32 x, u32 y) const
{
        ...
        case ECF_R8G8B8: {
                // NOTE: ECF_R8G8B8 stores bytes in memory order [R][G][B] per pixel (3 bytes).
...
void CImage::fill(const SColor &color)
{
        ...
        default:
                // NOTE: Other color formats (e.g., floating-point formats like ECF_R32F,
                // ECF_R16F, ECF_R16G16B16A16F, ECF_R32G32B32A32F) are not handled by fill()
                // and will silently fall through, leaving the image data unchanged. To add
                // support: implement per-pixel conversion from SColor to the target format,
                // or convert the image to a supported format first and then fill.
                return;
        }
