...
bool CImageWriterPNG::writeImage(io::IWriteFile *file, IImage *image, u32 param) const
{
        ...
        case ECF_A1R5G5B5:
                lineWidth *= 4;
                break;
        // NOTE: If the image format is not one of the handled cases below (ECF_R8G8B8,
        // ECF_A8R8G8B8, ECF_R5G6B5, ECF_A1R5G5B5), the lineWidth calculation will be
        // wrong (0 bytes per pixel) and the output PNG will be corrupt. Should return
        // false here for unsupported formats instead of falling through.
        default:
                break;
        }
        ...
        case ECF_A1R5G5B5:
                CColorConverter::convert_A1R5G5B5toA8R8G8B8(data, num_pixels,
                        tmpImage.get());
                break;
                // NOTE: The second switch already handles the unsupported-format case by
                // logging a warning and returning false, so the default branch is correct.
        default:
