...
EM_BOOL CIrrDeviceSDL::MouseEnterCallback(int eventType, const EmscriptenMouseEvent *mouseEvent, void *userData)
{
        ...
        irrevent.MouseInput.ButtonStates = This->MouseButtonStates; // NOTE: ButtonStates is copied from the device's tracked state rather than from mouseEvent->buttons because the Emscripten mouse event button bitfield (mouseEvent->buttons) was not successfully mapped to Irrlicht's button bit format. If the correct mapping is found, replace This->MouseButtonStates with the converted mouseEvent->buttons value for more accurate button state reporting during enter/leave events.
