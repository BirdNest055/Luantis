...
private:
        ...
        // NOTE: BUG — when the scene node has non-uniform scaling with components < 1.0, BBoxSafe
        // can be too small because it is calculated assuming scale >= 1.0. The billboard mesh
        // vertices are computed in world space during render() (via updateMesh()), so the bounding
        // box should also be computed in world space. The proper fix is to calculate the billboard
        // mesh vertices in *local* space (relative to the node's transform) and let the scene
        // manager's getTransformedBoundingBox() apply the node's scaling. This requires:
        //   (a) Changing updateMesh() to produce local-space vertices instead of world-space.
        //   (b) Computing BBoxSafe from the local-space mesh extent.
        //   (c) Removing the camera-dependent bounding box hack.
        // Additionally, we can't use the real camera-dependent bounding box for culling because
        // the camera used to orient the billboard is not yet updated at culling time — so we only
        // know the actual bounding box after rendering, which is too late for frustum culling.
