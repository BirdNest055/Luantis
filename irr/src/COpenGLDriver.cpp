...
bool COpenGLDriver::endScene()
{
        ...
        // NOTE: Console device present — no additional handling needed
...
{
        {
                ...
                // NOTE: Drawing GL_POINTS after GL_LINES is a workaround for the OpenGL
                // "diamond exit rule" — line rasterization may skip the first and last pixel
                // of a line segment. The point draw ensures those pixels are filled. However,
                // this has a side effect: the points are drawn with the current blend state,
                // which may produce visible artifacts with alpha blending (the point and line
                // fragments overlap, doubling the alpha at endpoints). If artifacts appear,
                // alternatives are: (a) use thick lines (GL_LINE_WIDTH > 1) which don't have
                // the diamond-exit issue, or (b) use a geometry shader to add endpoint coverage.
