...
bool CXMeshFileLoader::parseDataObjectMesh(SXMesh &mesh)
{
        ...
        for (u32 k = 0; k < nFaces; ++k) {
                ...
                if (fcnt != 3) {
                        ...
                        // NOTE: For n-gon faces (IndexCountPerFace > 3), the polygon is
                        // fan-triangulated above, but the per-face material index mapping
                        // is not updated to account for the additional triangles. Each
                        // n-gon face generates (n-2) triangles but the material list still
                        // has one entry per original face. Fix: when building the material
                        // index list, repeat the material index for each generated triangle.
                } else {
        ...
        while (true) {
                ...
                if (objectName == "MeshNormals") {
                        ...
                        // NOTE: VertexDuplicationIndices are ignored here. The DirectX .x format
                        // uses this object to specify vertex sharing across multiple faces (i.e.,
                        // vertices that should be treated as the same position but may have different
                        // UVs/normals). Loading them would allow merging duplicate vertices, reducing
                        // memory usage. However, most .x exporters don't emit this object, and the
                        // current mesh processing handles shared vertices via the index buffer.
                        if (!parseUnknownDataObject())
                                return false;
...
bool CXMeshFileLoader::parseDataObjectAnimation()
{
        ...
        while (true) {
                ...
                if (objectName.size() == 0) {
                        ...
                        // NOTE: AnimationOptions (looping flags: once, loop, ping-pong) are
                        // not parsed. This means all .x animations play with the default loop
                        // behavior. To implement: parse the "open" and "closed" keywords that
                        // follow AnimationOptions in the .x file, and store them as a loop-mode
                        // enum on the animation data. The DX spec defines:
                        //   AnimationOptions { open|closed, once|loop|pingpong }
                        if (!parseUnknownDataObject())
                                return false;
                } else if (objectName == "{") {
                        // read frame name
...
f32 CXMeshFileLoader::readFloat()
{
        if (BinaryFormat) {
                ...
                if (FloatSize == 8) {
#ifdef __BIG_ENDIAN__
                        // NOTE: Big-endian f64→f32 conversion below uses a two-step approach:
                        // byte-swap both halves of the f64 into a temporary, then cast to f32.
                        // This should be correct but has not been verified on a big-endian system.
                        // If this code is ever used on big-endian hardware, test with known values
                        // (e.g., 1.0, -0.5, 3.14159) and compare against little-endian output.
                        // The cast *(f64 *)(void *)ctmp is technically UB (type punning via cast);
                        // use memcpy for strict aliasing compliance if revisiting.
                        f32 ctmp[2];
