...
// NOTE: setTextRect() interprets VAlign according to line-index (the indexed line is placed
// on top/center/bottom), but HAlign according to pixel-width and not by row. Intuitively the
// HAlign handling is correct because VScrollPos already handles line-scrolling, so horizontal
// alignment should be pixel-based. Do not change VAlign behavior without also rewriting (and
// thoroughly testing) the auto-scrolling logic — the previous attempt to fix this broke
// auto-scrolling due to the interaction between VAlign line-indexing and HScrollPos updates.
...
void CGUIEditBox::calculateScrollPos()
{
        ...
        // NOTE: Calculations different to vertical scrolling because setTextRect interprets VAlign relative to line but HAlign not relative to row
        {
                ...
                if (txtWidth < FrameRect.getWidth()) {
                        // NOTE: When text width is smaller than the clipping area, HScrollPos is reset
                        // to 0, but there is no symmetric cleanup for left/right gaps based on HAlign.
                        // For example, with HAlign=Center, after scrolling right and then making the text
                        // shorter, the text snaps left instead of centering. A proper fix would compute
                        // HScrollPos based on HAlign: HScrollPos = max(0, (txtWidth - FrameRect.getWidth())
                        // * halign_factor), where halign_factor is 0.0 for left, 0.5 for center, 1.0 for
                        // right alignment. This would also apply when text is wider than the frame.
                ...
                if (CurrentTextRect.UpperLeftCorner.X + cStart < FrameRect.UpperLeftCorner.X) {
                        ...
                        // NOTE: When scrolling left (cursor hits the left border), the scroll
                        // adjustment only ensures the cursor is visible at the edge. It should
                        // additionally scroll by one extra character width so the user can see
                        // context to the left of the cursor — similar to how right-scrolling
                        // naturally reveals upcoming text. Fix: after adjusting HScrollPos, also
                        // subtract a small margin (e.g., font->getDimension("W").Width) to reveal
                        // one more character to the left.
