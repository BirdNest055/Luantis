...
void COpenGLES2Driver::initFeatures()
{
        ...
        // NOTE: On GLES, debug label functions (glObjectLabel, glGetObjectLabel, etc.) are
        // suffixed with "KHR" (e.g., glObjectLabelKHR) because they come from the GL_KHR_debug
        // extension, not from a core version. Our GL function loader code ignores this suffix
        // requirement and loads the unsuffixed symbols, which violates the GLES spec (see #15830).
        // This happens to work on Mesa because its libGLESv2 exports both suffixed and unsuffixed
        // symbols, but it will fail on strict GLES implementations (e.g., Android, proprietary
        // drivers). Proper fix: the function loader should try the KHR-suffixed names first when
        // building for GLES, falling back to the unsuffixed names for desktop GL.
        if (Name.find("Mesa") != -1) {
