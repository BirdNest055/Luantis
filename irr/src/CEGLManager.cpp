...
EGLConfig CEGLManager::chooseConfig(EConfigStyle confStyle)
{
        ...
        if (confStyle == ECS_EGL_CHOOSE_FIRST_LOWER_EXPECTATIONS) {
                ...
                // NOTE: eglChooseConfig returns the config that best matches the attribute
                // list, but "best" means the one with the most capabilities (e.g., highest
                // color depth), not the closest to the requested parameters. A future
                // confStyle ECS_EGL_CHOOSE_CLOSEST could iterate over all returned configs
                // and select the one whose attribute values are closest to the requested
                // values (minimizing over-specified features like 32-bit color when 16-bit
                // was requested). This would reduce unnecessary GPU memory bandwidth usage
                // on embedded devices where lower bit depths are preferred.
                //
