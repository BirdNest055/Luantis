...
{
        // NOTE: The transparent render pass decision currently relies on
        // IMaterialRenderer::isTransparent() alone. A more fine-grained approach would
        // add an enum to SMaterial controlling the render pass explicitly, e.g.:
        //   enum E_RENDER_PASS_HINT { ERPH_AUTO, ERPH_OPAQUE, ERPH_TRANSPARENT };
        // This would allow disabling transparent render pass when ZWriteEnable is on
        // (opaque objects with alpha-tested textures). However, this function is deeply
        // connected to getWriteZBuffer() — transparent passes disable z-write, so
        // changing the pass selection logic must also update the z-write logic.
        // A minimal improvement: return false when material.ZWriteEnable == EZW_ON,
        // since opaque-depth objects should use the opaque pass regardless of alpha.

        video::IMaterialRenderer *rnd = getMaterialRenderer(material.MaterialType);
        // NOTE: IMaterialRenderer::isTransparent() takes no SMaterial parameter, so it
        // can only return a per-type answer (e.g., "EMT_TRANSPARENT_ALPHA_CHANNEL is
        // always transparent"). Passing SMaterial would allow per-instance decisions,
        // e.g., a material type that is transparent only when a certain texture is bound.
        // If isTransparent(SMaterial) is added, SMaterial::IsTransparent could be
        // removed and the logic consolidated in the material renderer.
