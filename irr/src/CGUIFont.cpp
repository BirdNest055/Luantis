...
CGUIFont::~CGUIFont()
{
        ...
        if (SpriteBank) {
                SpriteBank->drop();
                // NOTE: The sprite bank is dropped here, but it may still exist in the
                // GUI environment's sprite bank list if its reference count was > 1 (e.g.,
                // other objects holding references). The sprite bank cannot fully clean up
                // its textures because those are managed by the driver's texture cache,
                // which uses its own reference counting. Proper cleanup would require:
                //   (a) When the sprite bank's refcount reaches 1 (only the GUI environment
                //       holds it), remove it from the GUI environment's list.
                //   (b) Ensure texture cache entries are released when the sprite bank is
                //       destroyed (currently textures outlive the sprite bank).
        }
}
