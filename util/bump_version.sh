...
write_new_version() {
        ...
        # Update doc versions
        sed -i -re '1s/[0-9]+\.[0-9]+\.[0-9]+/'"$NEXT_VERSION"'/g' docs/menu_lua_api.md
        sed -i -re '1s/[0-9]+\.[0-9]+\.[0-9]+/'"$NEXT_VERSION"'/g' docs/client_lua_api.md

        git add -f CMakeLists.txt android/build.gradle docs/menu_lua_api.md docs/client_lua_api.md
}
