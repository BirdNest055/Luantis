...
function contentdb.get_formspec_padding()
        ...
        -- NOTE: Currently uses a fixed larger padding on Android because we cannot
        -- determine the actual notch/cutout size. To properly handle this:
        -- 1. Use the Android WindowInsets API via JNI to get the exact display cutout
        --    bounds: WindowInsets.getDisplayCutout() → getBoundingRects().
        -- 2. Pass the inset values (in pixels) to the Lua layer through a new
        --    core.get_display_insets() function (or extend get_window_info()).
        -- 3. Convert pixel insets to formspec coordinates using the known scaling
        --    factor, and use them as dynamic padding instead of the fixed 1.0/0.25.
        -- See also: Android developer docs on DisplayCutout and WindowInsets.
