...
local function load_mod_command_tree()
        ...
        for name, def in pairs(core.registered_chatcommands) do
                ...
                -- NOTE: Commands are stored as {name, def} pairs without filtering by
                -- privileges here. Privilege checks are deferred to the formspec rendering
                -- phase (build_chatcommands_formspec) where the player's privileges are
                -- known, allowing the same mod_cmds table to serve all players.
