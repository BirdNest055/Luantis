...
local security_info_component = dofile(path .. "security_info_component.lua")
...
local function load()
        ...
        -- v8: Insert the security info component at the top of the auto-generated
        -- "Security Info" page (created by [Security Info] in settingtypes.txt).
        -- This page is a top-level section with its own sidebar entry,
        -- making it prominent rather than buried under Client → Connection Security.
        do
                local sec_page = page_by_id.security_info
                if sec_page then
                        -- Insert the security info panel at the top of the page, before settings
                        table.insert(sec_page.content, 1, security_info_component)
                end
        end

        -- Also keep the security info panel in the Connection Security sub-page
        -- for users who navigate there from the Client section
        do
                local sec_page = page_by_id.client_connection_security
                if sec_page then
                        table.insert(sec_page.content, 1, security_info_component)
                end
        end
...
local function get_formspec(dialogdata)
        ...
        local fs = {
                ...
                -- NOTE: Toggling a trivial difference (trailing space) forces the engine
                ...
                -- Root cause: showFormspec() uses string identity comparison (==) to skip
                -- redundant updates. When the same tab is re-selected, the formspec string
                -- is unchanged, so the engine never re-renders the content.
                -- Removal: Requires engine support for formspec versioning (e.g., an
                -- incrementing counter passed alongside the formspec string) or a
                -- "force_redisplay" boolean parameter in showFormspec(). See also:
                -- FormspecFormSource::m_formspec_string in guiFormSpecMenu.h.
