...
-- NOTE: Job cancellation (job:cancel()) does not remove the job from the heap or
-- linked list — it replaces the function with a no-op dummy_func and clears args.
-- This avoids the O(n) cost of finding and removing a job from the middle of the
-- heap. Cancelled jobs are still iterated during expiry processing, but their
-- dummy function executes instantly. For large numbers of cancellations, this
-- could cause unnecessary iteration; a future optimization could mark jobs as
-- cancelled and skip them in the expiry loop.
