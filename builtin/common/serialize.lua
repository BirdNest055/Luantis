...
function core.serialize(value)
        -- NOTE: Functions are still serialized (as a placeholder) but this is deprecated.
        -- The serialized output for a function is `dummy_func`, which means deserializing
        -- it produces an empty function — the original function body and upvalues are lost.
        -- This behavior exists for backward compatibility with mods that accidentally
        -- serialize tables containing functions. A future version should either:
        --   1. Raise an error instead of logging a deprecation warning, or
        --   2. Strip function entries entirely (serialize as nil) to make the
        --      data loss more obvious at deserialization time.
