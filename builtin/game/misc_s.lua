...
-- NOTE: This must match the implementation in src/script/common/c_converter.h.
-- The hash packs three signed 16-bit coordinates into a single 48-bit integer:
-- hash = (z+0x8000)<<32 | (y+0x8000)<<16 | (x+0x8000). The +0x8000 offset
-- converts from signed [-32768, 32767] to unsigned [0, 65535] range. Positions
-- outside the signed 16-bit range will produce incorrect hashes. This limits
-- the world to approximately ±32768 nodes on each axis.
