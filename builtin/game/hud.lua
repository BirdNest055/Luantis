...
-- NOTE: The return value of core.hud_replace_builtin is not documented in the
-- official Lua API (lua_api.md). It returns false if elem_name is not a
-- registered builtin HUD element (e.g., "health_bar", "breath_bar", "minimap",
-- "hotbar", "crosshair"). A future API documentation update should include this.
-- Additionally, calling this on a client that has not yet received the player's
-- HUD flags may not take effect until the next update cycle.
...
register_builtin_hud_element("minimap", {
        ...
        show_elem = function(player, flags)
                ...
                -- NOTE: Clients with protocol_version < 44 have a hardcoded minimap rendered
                -- in C++. Adding a Lua-defined minimap on top of that would produce a duplicate.
                -- Protocol 44+ uses the Lua-defined HUD minimap exclusively. This check can be
                -- removed once the minimum supported protocol version reaches 44.
                return flags.minimap and proto_ver >= 44
        end,
        update_def = function(player, elem_def)
                local proto_ver = core.get_player_information(player:get_player_name()).protocol_version
                -- NOTE: Protocol 45 introduced percentage-based HUD element sizing
                -- (negative size values = percentage of screen). Older clients use fixed
                -- pixel sizes (256x256). Like the proto_ver >= 44 check above, this can
                -- be removed once the minimum supported protocol reaches 45.
