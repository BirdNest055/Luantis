...
-- NOTE: VoxelArea uses 1-based flat-array indexing (index() returns 1 for the
-- first voxel at MinEdge). This matches the convention used by VoxelManip data
-- arrays in the C++ engine. The ystride and zstride are computed from the area
-- extent (not from arbitrary values), ensuring that index() is always consistent
-- with the flat memory layout of the VoxelManip buffer.
VoxelArea = {
