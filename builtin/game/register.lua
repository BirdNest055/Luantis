...
function core.register_item(name, itemdef)
        ...
        -- NOTE: These metatable checks detect a class of mod bugs where item
        -- definition tables are reused across registrations or have unexpected
...
function core.override_item(name, redefinition, del_fields)
        ...
        if itemdefs_finalized then
                -- NOTE: Overriding items after server startup is unsupported because other
                -- parts of the engine may have cached the original definition (e.g., craft
                -- recipes, alias maps, node timers). It remains allowed because some mods
                -- depend on it for hot-reload workflows, but it should not be relied upon.
                -- If a need for runtime overrides is proven, a formal API should be added
                -- that also invalidates relevant caches (craft results, node def caches, etc.).
