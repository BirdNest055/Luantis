...
core.builtin_auth_handler = {
        ...
        set_privileges = function(name, privileges)
                ...
                for priv, value in pairs(privileges) do
                        -- NOTE: The privilege table uses a set-of-keys convention where only
                        -- keys with value `true` represent granted privileges. Passing `false`
                        -- is a common mod bug — it *adds* the key to the set, effectively
                        -- granting the privilege instead of revoking it. To revoke, the key
                        -- must be omitted entirely (set to nil). This is logged as deprecated
                        -- because the API contract is counterintuitive: a future version should
                        -- either reject non-true values with an error, or treat false as revocation.
                        if value == false then
