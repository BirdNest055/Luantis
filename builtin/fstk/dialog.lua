...
-- NOTE: Dialogs in fstk follow a parent-child stack pattern. When a dialog is
-- deleted (e.g., on ESC), its parent is automatically re-shown. The `hidden` flag
-- prevents interaction with a background dialog while a child is displayed.
-- Dialogs start hidden (self.hidden = true) and must be explicitly shown via :show().
-- The default event handler closes the dialog on MenuQuit (ESC) unless a custom
-- user_eventhandler returns true to consume the event.
